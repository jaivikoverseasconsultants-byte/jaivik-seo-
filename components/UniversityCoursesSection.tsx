...
type Level = 'All' | 'Undergraduate' | 'Postgraduate' | 'MBA' | 'PhD';
...
function deriveLevel(name: string): Exclude<Level, 'All'> {
...
export default function UniversityCoursesSection({
  ...
  const enriched = popularCourses.map(name => ({
    name,
  ...
  const counts: Record<Level, number> = {
    All: enriched.length,
    Undergraduate: enriched.filter(c => c.level === 'Undergraduate').length,
    Postgraduate: enriched.filter(c => c.level === 'Postgraduate').length,
    MBA: enriched.filter(c => c.level === 'MBA').length,
    PhD: enriched.filter(c => c.level === 'PhD').length,
  };

  const tabs: Level[] = (['All', 'Undergraduate', 'Postgraduate', 'MBA', 'PhD'] as Level[]).filter(
    l => l === 'All' || counts[l] > 0
  );

  const displayed = activeLevel === 'All' ? enriched : enriched.filter(c => c.level === activeLevel);
        ...
        {displayed.map(course => (
          ...
          >
            <div className="flex items-start justify-between gap-2 mb-3">
              <p className="text-sm font-semibold text-gray-900 group-hover:text-brand-700 leading-snug">
                {course.name}
              </p>
              <span className={`text-xs px-2 py-0.5 rounded-full whitespace-nowrap flex-shrink-0 font-medium ${LEVEL_STYLES[course.level]}`}>
                {course.level}
              </span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-xs">
              ...
              <div>
                ...
                <p className="font-semibold text-brand-700">${(annualTuitionUSD / 1000).toFixed(0)}K/yr</p>
              </div>
              <div>
                <p className="text-gray-400 mb-0.5">IELTS Min</p>
                <p className="font-semibold text-gray-700">{ieltsMin}+</p>
              </div>
            </div>
      ...
      <div className="mt-5 flex items-center justify-between bg-brand-50 rounded-xl p-4 border border-brand-100">
        <div>
          <p className="text-sm font-semibold text-brand-900">
            {popularCourses.length} courses listed · Full catalogue at {uniName}
          </p>
