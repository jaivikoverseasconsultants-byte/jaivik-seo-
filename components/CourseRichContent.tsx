...
export default function CourseRichContent({ course, universityName, universitySlug }: Props) {
  const { whyStudyHere } = generateCourseContent(
    course,
    universityName,
    universitySlug,
  );
  ...
  return (
    <>
      ...
      {/* Overview, Fees Breakdown, Entry Requirements, PSW & PR Pathway, Career Outcomes —
          all computed from real course/university data only; each section skips itself if its data is missing. */}
      <CourseKeyFacts course={course} universityName={universityName} universitySlug={universitySlug} />
      ...
      {/* Indian Students: Intake & Scholarships */}
      <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
        <h2 className="text-xl font-bold text-gray-900 mb-4">
          {course.name} for Indian Students: Intake &amp; Scholarships
        </h2>
