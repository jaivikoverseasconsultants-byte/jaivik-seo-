...
**Known pre-existing issue surfaced by this cleanup, not fixed here:** `app/universities/[slug]/page.tsx` displays `visaApprovalRate`, `acceptanceRate`, and `employmentRate` as bare factual-sounding stats (including inside `FAQPage` JSON-LD structured data, e.g. *"The student visa approval rate for {country} is approximately {rate}% for Indian students..."*) with no "indicative/estimated" labelling anywhere in the UI — for all 458+ entries in this file, not just the 7 added here. Flagged for a follow-up task to either relabel these fields as indicative or remove them from display; out of scope for this cleanup.
