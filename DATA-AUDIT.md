...
**Neither finding has been fixed** — both are reported for a future decision, not resolved. Do not assume `ieltsMin`/`toeflMin`/`pteMin` are real for any given university without independently re-checking its generator script, and do not assume every "course" in `cdu-courses.ts` is actually a course.
