...
2. **`data/cdu-courses.ts` (Charles Darwin University, REAL, 405 courses) contains a meaningful number of scraped site-navigation entries that aren't courses at all** ("Ask a question", "Find a course", etc.) — see its row in the table below. The REAL classification's "unique deep URL" test doesn't distinguish a course page from a navigation page if both have deep, unique URLs, which is exactly what happened here.

**Finding 2 (CDU) is still unfixed** — reported for a future decision, not resolved; do not assume every "course" in `cdu-courses.ts` is actually a course. **Finding 1 (ieltsMin/toeflMin/pteMin) got a Phase 1 UI fix 2026-07-18** — the 3 IELTS-band decision hubs no longer display a per-course table built on this field (see BUILD-LOG.md §1 "Decision hub pages" and §2 item 12) — but **the underlying data field itself is untouched and still just as unreliable**. Do not assume `ieltsMin`/`toeflMin`/`pteMin` are real for any given university without independently re-checking its generator script, wherever else in the app this field is still read (`SubjectPillarPage.tsx`'s per-country "min IELTS" stat, `CostPillarPage.tsx`'s IELTS-adjacent copy, per-course pages' own IELTS display, and anywhere else — none of those were touched by the Phase 1 hub fix). A Phase 2 real university-level English-requirements crawl is the actual fix for the data itself and hasn't started.
...
| Charles Darwin University | Australia | REAL | 405 | No header, but all 405 courses have unique deep institution URLs (e.g. https://www.cdu.edu.au/study/were-here-to-help-you), non-templated count. **CAVEAT added 2026-07-18 (BUILD-LOG.md §2 item 14):** that example URL is itself the problem — `/study/were-here-to-help-you` is a site-navigation page, not a course. A keyword check (course name contains bachelor/master/diploma/certificate/graduate/phd/doctor) flags 160 of the 405 "courses" (39.5%, an overestimate — some flagged entries are plausibly real) as lacking any degree-level keyword; confirmed non-course examples include "Ask a question", "Need to know more", "Find a course", "English proficiency", "How we rank", "TAFE course enquiry". The REAL classification here was based on URL uniqueness/depth, which a broad sitemap scrape of navigation pages also satisfies — URL uniqueness alone doesn't confirm the URL is a course page. **Not re-audited or fixed — needs a dedicated pass** (check each of the 405 URLs actually resolves to a course, not a nav/info page, before trusting this row's REAL status at face value). |
