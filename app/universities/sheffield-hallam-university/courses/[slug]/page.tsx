...
import { sheffieldHallamCourses, getSheffieldHallamCourseBySlug } from '@/data/sheffield-hallam-courses';
...
const UNIVERSITY_SLUG = 'sheffield-hallam-university';
export function generateStaticParams() {
  return sheffieldHallamCourses.map(c => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getSheffieldHallamCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Sheffield Hallam University — ${titleFeeFragment(course as any, course.annualINR)}IELTS & Requirements for Indian Students`,
    description: `${course.name} at Sheffield Hallam University, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)} IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/sheffield-hallam-university/courses/${slug}`,
    keywords: [course.name, 'SHU', 'Sheffield Hallam University', 'study in UK', course.level],
  });
}
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/sheffield-hallam-university" className="hover:text-white">SHU</Link> /
            <Link href="/universities/sheffield-hallam-university/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at Sheffield Hallam University — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`sheffield_hallam_university-course-${slug}`} defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            {entryRequirementsVaryByCourse(UNIVERSITY_SLUG) ? (
              ...
              <p className="text-sm text-gray-600">
                Sheffield Hallam University publishes one English language requirement across its courses
          ...
          <CourseRichContent course={course as any} universityName="Sheffield Hallam University" universitySlug="sheffield-hallam-university" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`sheffield_hallam_university-course-${slug}-sidebar`} defaultCountry="UK" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/sheffield-hallam-university/courses" className="block text-sm text-brand-700 hover:underline">
                  All SHU Courses →
                </Link>
