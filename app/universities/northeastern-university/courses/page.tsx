...
export const metadata: Metadata = buildMetadata({
  ...
  description: '39 programs at Northeastern University for Indian students. Tuition, IELTS requirements, intake dates. Free counselling by Jaivik Overseas.',
  path: '/universities/northeastern-university/courses',
});
...
export default function NortheasternCoursesPage() {
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does Northeastern University offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Northeastern University offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at Northeastern University?`,
        acceptedAnswer: {
          '@type': 'Answer',
      ...
      ...(_avgFeeUSD > 0 ? [{
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at Northeastern University is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * RATE_TO_INR.USD / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      }] : []),
      {
        '@type': 'Question',
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Northeastern University offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to Northeastern University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to Northeastern University in USA through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `Northeastern University — Courses for International Students`,
    description: `${courses.length} programs at Northeastern University, USA. Min IELTS ${_minIelts}+.`,
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-6">
        ...
        <Link href="/universities/northeastern-university" className="hover:text-brand-700">Northeastern University</Link> /
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-3">
          {courses.map(c => (
            <Link key={c.id} href={`/universities/northeastern-university/courses/${c.slug}`}
