...
export async function generateMetadata(
  ...
  const c = northeasternCourses.find(x => x.slug === slug);
  if (!c) return {};
  return buildMetadata({
    title: `${c.name} at Northeastern University — ${titleFeeFragment(c as any, c.annualINR)}IELTS & Requirements for Indian Students`,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-6 flex-wrap">
        ...
        <Link href="/universities/northeastern-university/courses" className="hover:text-brand-700">Courses</Link> /
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">{c.name} at Northeastern University — {titleFeeFragment(c as any, c.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
            <p className="text-gray-500 text-sm">Northeastern University · {c.city}, {c.state}</p>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
              {[
                { label: 'Duration', value: c.duration },
                { label: 'Level', value: c.level },
                c.annualUSD > 0 ? { label: 'Annual Fee (USD)', value: feeDisplay(c as any, c.annualUSD, 'USD') } : null,
                c.annualINR > 0 ? { label: 'Annual Fee (INR)', value: feeDisplayINRLakh(c as any, (courseAnnualINRLakh(c as any, 1) ?? '0'), '') } : null,
          ...
          <div className="bg-brand-700 rounded-2xl p-6 text-white text-center">
            <h2 className="text-lg font-bold mb-2">Apply for {c.name} at Northeastern University</h2>
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="northeastern-university-course" defaultCountry="USA" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">More Programs at Northeastern University</h3>
              <div className="space-y-2">
                {northeasternCourses.filter(x=>x.slug!==slug).slice(0,6).map(x=>(
                  <Link key={x.id} href={`/universities/northeastern-university/courses/${x.slug}`}
                    className="block text-sm text-brand-700 hover:underline">
                    {x.name} →
                  </Link>
                ))}
              </div>
              <Link href="/universities/northeastern-university/courses" className="block text-xs text-gray-500 hover:underline mt-3">
                All Northeastern University programs →
              </Link>
            </div>
          </div>
        </div>
