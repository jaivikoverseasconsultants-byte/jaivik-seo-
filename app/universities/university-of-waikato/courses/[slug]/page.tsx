...
import { waikatoCourses, getWaikatoCourseBySlug } from '@/data/waikato-courses';
...
export async function generateStaticParams() {
  return (waikatoCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getWaikatoCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at University of Waikato — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at University of Waikato, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/university-of-waikato/courses/${slug}`,
    keywords: [course.name, 'Waikato', 'University of Waikato', 'study in New Zealand', course.level],
  });
}
...
export default async function CoursePage(
  ...
  const schema = {
    ...
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'University of Waikato', sameAs: 'https://www.waikato.ac.nz' },
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-waikato" className="hover:text-white">Waikato</Link> /
            <Link href="/universities/university-of-waikato/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`waikato-course-${slug}`} defaultCountry="New Zealand" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                { label: `Tuition × ${course.durationYears} yr${course.durationYears !== 1 ? 's' : ''}`, value: `NZ$${course.totalNZD.toLocaleString()}`, hi: true },
                ...
                { label: 'Total Est. Cost', value: `NZ$${(course.totalNZD + course.livingCostNZD * course.durationYears).toLocaleString()}`, hi: true },
                { label: 'In Indian Rupees', value: `₹${((course.totalNZD + course.livingCostNZD * course.durationYears) * 0.60 * 84 / 100000).toFixed(1)} Lakh`, hi: true },
              ].map(r => (
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Visa & Work Rights — New Zealand</h2>
          ...
          <CourseRichContent course={course as any} universityName="University of Waikato" universitySlug="university-of-waikato" />
          <div className="bg-brand-700 rounded-2xl p-6 text-white text-center">
            ...
            <p className="text-blue-200 text-sm mb-4">Our New Zealand admissions advisors give free, personalised guidance to Indian students.</p>
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`waikato-course-${slug}-sidebar`} defaultCountry="New Zealand" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/university-of-waikato/courses" className="block text-sm text-brand-700 hover:underline">All Waikato Courses →</Link>
                <Link href="/universities/country/new-zealand" className="block text-sm text-brand-700 hover:underline">Study in New Zealand Guide →</Link>
              </div>
            </div>
          </div>
        </div>
