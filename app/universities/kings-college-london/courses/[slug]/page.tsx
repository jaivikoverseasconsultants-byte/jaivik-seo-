...
export async function generateMetadata(
  ...
  return buildMetadata({
    ...
    description: `${course.name} at King's College London, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
            ...
            {entryRequirementsVaryByCourse(UNIVERSITY_SLUG) && publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0 ? (
              ...
              <p className="text-sm text-gray-600">
                {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0
                  ? "Kings College London publishes one English language requirement across its courses rather than a per-course score."
                  : englishOnRequestNote("Kings College London")}{' '}
