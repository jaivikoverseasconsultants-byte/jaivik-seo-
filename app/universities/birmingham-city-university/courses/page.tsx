...
import { bcuW2Courses } from '@/data/bcu-w2-courses';
import LeadForm from '@/components/LeadForm';
import JsonLd from '@/components/JsonLd';
import { verifiedAvgFee } from '@/lib/fee-verification';
import { hasPublishedIelts } from '@/lib/english-verification';
const UNIVERSITY_SLUG = 'birmingham-city-university';

export const metadata: Metadata = buildMetadata({
  title: 'Birmingham City University Courses — Fees & IELTS 2026',
  description: `Birmingham City University – ${bcuW2Courses.length} courses for international students. Free guidance from Jaivik Overseas Consultants.`,
  path: '/universities/birmingham-city-university/courses',
  keywords: ['BCU courses', 'Birmingham City University', 'study in UK'],
});

function groupByLevel(courses: typeof bcuW2Courses) {
  const groups: Record<string, typeof bcuW2Courses> = {};
  courses.forEach((c) => { const lv = c.studyLevel || 'Other'; if (!groups[lv]) groups[lv] = []; groups[lv].push(c); });
  return groups;
}

export default function CoursesPage() {
  const courses = bcuW2Courses;
  ...
  const orderedGroups = levelOrder.filter(l => groups[l]).map(l => [l, groups[l]] as [string, typeof bcuW2Courses]);
  const allGroups = [...orderedGroups, ...Object.keys(groups).filter(l => !levelOrder.includes(l)).map(l => [l, groups[l]] as [string, typeof bcuW2Courses])];

  const schema = { '@context': 'https://schema.org', '@type': 'CollegeOrUniversity', name: 'Birmingham City University', sameAs: 'https://www.bcu.ac.uk', url: 'https://www.bcu.ac.uk' };
  const courseListSchema = {
    ...
    itemListElement: courses.slice(0, 5).map((c, i) => ({
      ...
      item: { '@type': 'Course', name: c.name, provider: { '@type': 'CollegeOrUniversity', name: 'Birmingham City University' }, educationalLevel: c.studyLevel },
    })),
  };
  ...
  return (
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/birmingham-city-university" className="hover:text-white">BCU</Link> /
            <span>Courses</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-bold mb-3">Birmingham City University</h1>
          <p className="text-blue-100 text-lg mb-6">{courses.length} Programs · Birmingham, UK</p>
          ...
          <div key={level} className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {levelCourses.map((course) => {
                ...
                return (
                  <Link key={course.id} href={`/universities/birmingham-city-university/courses/${course.slug}`}
      ...
      <section className="bg-brand-50 py-12 px-4"><div className="max-w-2xl mx-auto"><h2 className="text-2xl font-bold text-center text-brand-900 mb-2">Apply to BCU</h2><p className="text-center text-gray-600 mb-6">Get free expert guidance from Jaivik Overseas Consultants</p><LeadForm /></div></section>
    </>
  );
}
