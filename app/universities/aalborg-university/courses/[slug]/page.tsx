...
import { aalborgUniversityCourses, getAalborgUniversityCourseBySlug } from '@/data/aalborg-university-courses';
...
export async function generateStaticParams() {
  return (aalborgUniversityCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getAalborgUniversityCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Aalborg University — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at Aalborg University, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/aalborg-university/courses/${slug}`,
    keywords: [course.name, 'Aalborg University', 'study in Denmark', course.level],
  });
}
...
export default async function CourseDetailPage(
  ...
  const schema = {
    ...
    '@type': 'Course',
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'Aalborg University', sameAs: 'https://www.aau.dk' },
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/aalborg-university/courses" className="hover:text-white">AAU</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="aalborg-university-${slug}" defaultCountry="Denmark" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <CourseRichContent course={course as any} universityName="Aalborg University" universitySlug="aalborg-university" />
        ...
        <div>
          <div className="sticky top-20">
            <LeadForm source="aalborg-university-detail-sidebar" defaultCountry="Denmark" />
          </div>
        </div>
      </div>
    </>
