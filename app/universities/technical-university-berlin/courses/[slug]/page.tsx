...
import { tubCourses, getTubCourseBySlug } from '@/data/tub-courses';
...
export async function generateStaticParams() {
  return (tubCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getTubCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Technical University of Berlin — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at Technical University of Berlin, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/technical-university-berlin/courses/${slug}`,
    keywords: [course.name, 'TU Berlin', 'Technical University of Berlin', 'study in Germany', course.level],
    noIndex: true
  });
}
...
export default async function CoursePage(
  ...
  const schema = {
    ...
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'Technical University of Berlin', sameAs: 'https://www.tu.berlin/en/' },
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/technical-university-berlin" className="hover:text-white">TU Berlin</Link> /
            <Link href="/universities/technical-university-berlin/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`tub-course-${slug}`} defaultCountry="Germany" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { label: 'Post-Study Visa', value: '18-month Job Seeker Visa post-study' },
              ].map(f => (
                <div key={f.label} className="p-4 bg-gray-50 rounded-xl">
                  <p className="text-xs text-gray-500 font-medium mb-1">{f.label}</p>
                  <p className="text-sm font-semibold text-gray-900">{f.value}</p>
          ...
          <CourseRichContent course={course as any} universityName="Technical University of Berlin" universitySlug="technical-university-berlin" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`tub-course-${slug}-sidebar`} defaultCountry="Germany" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/technical-university-berlin/courses" className="block text-sm text-brand-700 hover:underline">All TU Berlin Courses →</Link>
