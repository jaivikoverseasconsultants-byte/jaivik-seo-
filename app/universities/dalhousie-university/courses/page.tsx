...
import { dalCourses } from '@/data/dal-courses';
...
const UNIVERSITY_SLUG = 'dalhousie-university';

export const metadata: Metadata = buildMetadata({
  title: 'Dal Courses — Fees & IELTS 2026',
  description: `Dalhousie University — ${(dalCourses as unknown as any[]).length} courses for international students. PGWP Eligible · IELTS 6.5+. September & January intakes. Free admission guidance from Jaivik Overseas Consultants.`,
  path: '/universities/dalhousie-university/courses',
  keywords: ['Dal courses', 'Dalhousie University international', 'Dal fees', 'study in Canada', 'Canada university', 'PGWP'],
});
...
export default function CoursesPage() {
  const courses = dalCourses as unknown as any[];
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does Dalhousie University offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Dalhousie University offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at Dalhousie University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at Dalhousie University is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      ...(_avgFeeUSD > 0 ? [{
        '@type': 'Question',
        name: `What is the average tuition fee at Dalhousie University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at Dalhousie University is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * RATE_TO_INR.USD / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      }] : []),
      {
        '@type': 'Question',
        name: `What intake options does Dalhousie University offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Dalhousie University offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to Dalhousie University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to Dalhousie University in Canada through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `Dalhousie University — Courses for International Students`,
    description: `${courses.length} programs at Dalhousie University, Canada. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'Dalhousie University' },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇨🇦 Halifax, Canada · PGWP Eligible · #301 QS
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">
                Dalhousie University — International Courses
              </h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="dal-courses-index" defaultCountry="Canada" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div key={level}>
              ...
              <div className="space-y-3">
                {(lvCourses as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((c: any) => (
                  <Link key={c.slug} href={`/universities/dalhousie-university/courses/${c.slug}`}
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="dal-courses-sidebar" defaultCountry="Canada" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Quick Facts — Dal</h3>
              {[
                ['Established', '1818'],
