...
import { ucalgaryCourses, getUcalgaryCoursesBySlug } from '@/data/ucalgary-courses';
...
export async function generateStaticParams() {
  return (ucalgaryCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getUcalgaryCoursesBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at University of Calgary`,
    description: `${course.name} at University of Calgary, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/university-of-calgary/courses/${slug}`,
    keywords: [course.name, 'UCalgary', 'University of Calgary', 'study in Canada', course.level, 'PGWP'],
  });
}
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-calgary" className="hover:text-white">UCalgary</Link> /
            <Link href="/universities/university-of-calgary/courses" className="hover:text-white">Courses</Link> /
            <span className="text-white">{course.name}</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇨🇦 University of Calgary · Calgary, Canada · PGWP Eligible
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at University of Calgary — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`ucalgary-course-${slug}`} defaultCountry="Canada" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
            ...
            {entryRequirementsVaryByCourse(UNIVERSITY_SLUG) && publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0 ? (
              ...
              <p className="text-sm text-gray-600">
                {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0
                  ? "University Of Calgary publishes one English language requirement across its courses rather than a per-course score."
                  : englishOnRequestNote("University Of Calgary")}{' '}
          ...
          <CourseRichContent course={course as any} universityName="University of Calgary" universitySlug="university-of-calgary" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`ucalgary-course-${slug}-sidebar`} defaultCountry="Canada" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/university-of-calgary/courses" className="block text-sm text-brand-700 hover:underline">
                  All UCalgary Courses →
                </Link>
