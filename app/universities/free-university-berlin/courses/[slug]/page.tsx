...
import { fubCourses, getFubCourseBySlug } from '@/data/fub-courses';
import { buildMetadata } from '@/lib/seo';
import LeadForm from '@/components/LeadForm';
import JsonLd from '@/components/JsonLd';

export async function generateStaticParams() {
  return (fubCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getFubCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} | FU Berlin – Fees, IELTS & Intake 2025`,
    description: `${course.name} at Free University of Berlin. Annual fee €${course.annualEUR.toLocaleString()} (${course.durationYears} yr${course.durationYears !== 1 ? 's' : ''}). IELTS ${course.ieltsMin}+. Intake: ${course.intakeMonths.join(' & ')}. Free guidance from Jaivik Overseas.`,
...
export default async function CoursePage(
  ...
  const schema = {
    ...
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'Free University of Berlin', sameAs: 'https://www.fu-berlin.de/en/' },
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/free-university-berlin/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`fub-course-${slug}`} defaultCountry="Germany" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`fub-course-${slug}-sidebar`} defaultCountry="Germany" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/free-university-berlin/courses" className="block text-sm text-brand-700 hover:underline">All FU Berlin Courses →</Link>
