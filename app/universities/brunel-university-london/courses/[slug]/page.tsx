...
import { brunelW2Courses, getBrunelW2CourseBySlug } from '@/data/brunel-w2-courses';
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const course = getBrunelW2CourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Brunel University London — Fees in INR, IELTS & Requirements for Indian Students`,
    description: `${course.name} at Brunel University London, Uxbridge${course.annualINR > 0 ? ` costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students.` : '.'}${course.ieltsMin > 0 ? ` IELTS ${course.ieltsMin}+,` : ''} Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
...
export default async function CoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = { '@context': 'https://schema.org', '@type': 'Course', name: course.name, provider: { '@type': 'CollegeOrUniversity', name: 'Brunel University London', sameAs: 'https://www.brunel.ac.uk' }, courseMode: 'full-time', educationalLevel: course.studyLevel };
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/brunel-university-london" className="hover:text-white">Brunel</Link> /
            <Link href="/universities/brunel-university-london/courses" className="hover:text-white">Courses</Link> /
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at Brunel University London — Fees in INR, IELTS &amp; Requirements for Indian Students</h1>
          <p className="text-blue-100 text-lg">Brunel University London · Uxbridge, UK · {course.duration}</p>
        </div>
      </section>
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <CourseKeyFacts course={course as any} universityName="Brunel University London" universitySlug="brunel-university-london" />
          </div>
          <div className="space-y-6">
            <CourseRichContent course={course as any} universityName="Brunel University London" universitySlug="brunel-university-london" />
            ...
            <Link href="/universities/brunel-university-london/courses" className="flex items-center gap-2 text-brand-700 hover:text-brand-900 font-medium text-sm">← All Brunel Courses</Link>
          </div>
        </div>
      </section>
