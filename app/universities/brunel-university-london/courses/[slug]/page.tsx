...
import { brunelCourses, getBrunelCoursesBySlug } from '@/data/brunel-courses';
...
export async function generateStaticParams() {
  return (brunelCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getBrunelCoursesBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Brunel University London — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at Brunel University London, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`brunel-course-${slug}`} defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`brunel-course-${slug}-sidebar`} defaultCountry="UK" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/brunel-university-london/courses" className="block text-sm text-brand-700 hover:underline">
                  All Brunel Courses →
                </Link>
