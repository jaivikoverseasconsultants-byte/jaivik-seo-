...
import { edinburghCourses } from '@/data/edinburgh-courses';
...
export default function CoursesPage() {
  const courses = edinburghCourses as unknown as any[];
  ...
  const avgFee = pgCourses.length
    ? Math.round(pgCourses.reduce((s: number, c: any) => s + (c.annualGBP || c.annualUSD || 0), 0) / pgCourses.length)
    : Math.round(courses.reduce((s: number, c: any) => s + (c.annualGBP || c.annualUSD || 0), 0) / (totalCourses || 1));
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does University of Edinburgh offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `University of Edinburgh offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at University of Edinburgh?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at University of Edinburgh is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      {
        '@type': 'Question',
      ...
      {
        '@type': 'Question',
        name: `What intake options does University of Edinburgh offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `University of Edinburgh offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to University of Edinburgh?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to University of Edinburgh in United Kingdom through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `University of Edinburgh — Courses for International Students`,
    description: `${courses.length} programs at University of Edinburgh, United Kingdom. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'University of Edinburgh' },
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-edinburgh" className="hover:text-white">Edinburgh</Link> /
            <span>Courses</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-bold mb-3">University of Edinburgh</h1>
          <p className="text-blue-100 text-lg mb-6">
            {totalCourses} Programs for International Students · Edinburgh, UK · IELTS 6.0–7.0+
          </p>
          ...
          <div key={level} className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {levelCourses.map((course: any) => {
                const fee = course.annualGBP || course.annualUSD || 0;
                const feeStr = fee > 0 ? '£' + fee.toLocaleString() + '/yr' : 'Low fees';
                return (
                  <Link
                    key={course.id}
                    href={`/universities/university-of-edinburgh/courses/${course.slug}`}
      ...
      <section className="bg-brand-50 py-12 px-4">
        <div className="max-w-2xl mx-auto">
          <h2 className="text-2xl font-bold text-center text-brand-900 mb-2">Apply to Edinburgh</h2>
