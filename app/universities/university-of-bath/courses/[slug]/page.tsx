...
import { bathCourses, getBathCourseBySlug } from '@/data/bath-courses';
...
export async function generateStaticParams() {
  return (bathCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  return buildMetadata({
    title: `${course.name} at University of Bath`,
    description: `${course.name} at University of Bath, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/university-of-bath/courses/${slug}`,
    keywords: [course.name, 'Bath', 'University of Bath', 'study in UK', course.level],
  });
}
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-bath" className="hover:text-white">Bath</Link> /
            <Link href="/universities/university-of-bath/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`bath-course-${slug}`} defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0 && (entryRequirementsVaryByCourse(UNIVERSITY_SLUG) || !!(course as any).englishVerified) ? (
              ...
              <p className="text-sm text-gray-600">
                {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0
                  ? "University Of Bath publishes one English language requirement across its courses rather than a per-course score."
                  : englishOnRequestNote("University Of Bath")}{' '}
          ...
          <CourseRichContent course={course as any} universityName="University of Bath" universitySlug="university-of-bath" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`bath-course-${slug}-sidebar`} defaultCountry="UK" />
