...
import { thinkCourses } from '@/data/think-courses';
import LeadForm from '@/components/LeadForm';
import JsonLd from '@/components/JsonLd';

export const metadata: Metadata = buildMetadata({
  title: 'Think Education International Courses – All Programs, Fees & IELTS 2026',
  description: `Think Education — ${(thinkCourses as unknown as any[]).length} courses for international students. IELTS 5.5+. February & July intakes. Free admission guidance from Jaivik Overseas Consultants.`,
  path: '/universities/think-education-australia/courses',
  keywords: ['Think courses', 'Think Education international', 'Think fees', 'study in Australia', 'Australia university'],
});
...
export default function CoursesPage() {
  const courses = thinkCourses as unknown as any[];
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does Think Education offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Think Education offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at Think Education?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at Think Education is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the average tuition fee at Think Education?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at Think Education is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * 84 / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      },
      {
        '@type': 'Question',
        name: `What intake options does Think Education offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Think Education offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to Think Education?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to Think Education in Australia through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `Think Education — Courses for International Students`,
    description: `${courses.length} programs at Think Education, Australia. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'Think Education' },
  ...
  const schema = {
    ...
    name: 'Think Education', sameAs: 'https://www.think.edu.au',
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇦🇺 Sydney, Australia · QS 501+ World Ranking
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">
                Think Education — International Courses
              </h1>
              <p className="text-blue-200 text-lg mb-5">
                {totalCourses} programs · Avg A${avgFee.toLocaleString()}/yr · IELTS 5.5+ · February & July intakes
              </p>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="think-courses-index" defaultCountry="Australia" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div key={level}>
              ...
              <div className="space-y-3">
                {(lvCourses as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((c: any) => (
                  <Link key={c.slug} href={`/universities/think-education-australia/courses/${c.slug}`}
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="think-courses-sidebar" defaultCountry="Australia" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Quick Facts — Think</h3>
              {[
                ['Established', '2002'],
