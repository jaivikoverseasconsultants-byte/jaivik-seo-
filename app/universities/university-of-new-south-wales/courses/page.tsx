...
import { unswCourses } from '@/data/unsw-courses';
import LeadForm from '@/components/LeadForm';
import JsonLd from '@/components/JsonLd';

export const metadata: Metadata = buildMetadata({
  title: 'UNSW Sydney International Courses – All Programs, Fees & IELTS 2026',
  description: `UNSW Sydney – ${(unswCourses as unknown as any[]).length} courses for international students. Free guidance from Jaivik Overseas Consultants.`,
  path: '/universities/university-of-new-south-wales/courses',
  keywords: ['UNSW courses', 'UNSW Sydney international', 'study in Australia'],
});
...
export default function CoursesPage() {
  const courses = unswCourses as unknown as any[];
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does UNSW Sydney offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `UNSW Sydney offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at UNSW Sydney?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at UNSW Sydney is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the average tuition fee at UNSW Sydney?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at UNSW Sydney is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * 84 / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      },
      {
        '@type': 'Question',
        name: `What intake options does UNSW Sydney offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `UNSW Sydney offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to UNSW Sydney?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to UNSW Sydney in Australia through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    ...
    description: `${courses.length} programs at UNSW Sydney, Australia. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'UNSW Sydney' },
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-new-south-wales" className="hover:text-white">UNSW</Link> /
            <span>Courses</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-bold mb-3">UNSW Sydney</h1>
          <p className="text-blue-100 text-lg mb-6">
            {totalCourses} Programs for International Students · Sydney, Australia · IELTS 6.0–7.0+
          </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {levelCourses.map((course: any) => {
                ...
                return (
                  <Link
                    key={course.id}
                    href={`/universities/university-of-new-south-wales/courses/${course.slug}`}
      ...
      <section className="bg-brand-50 py-12 px-4">
        <div className="max-w-2xl mx-auto">
          <h2 className="text-2xl font-bold text-center text-brand-900 mb-2">Apply to UNSW</h2>
