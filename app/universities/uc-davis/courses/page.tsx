...
const UNIVERSITY_SLUG = 'uc-davis';

export const metadata: Metadata = buildMetadata({
  title: 'UC Davis Courses — Fees & IELTS 2026',
  description: '39 programs at UC Davis for Indian students. Tuition, IELTS requirements, intake dates. Free counselling by Jaivik Overseas.',
  path: '/universities/uc-davis/courses',
});

const levels = ['All', 'Bachelor', 'Master', 'Doctoral', 'Diploma'];

export default function UcdavisCoursesPage() {
  const courses = ucdavisCourses;
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does UC Davis offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `UC Davis offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at UC Davis?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at UC Davis is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      ...(_avgFeeUSD > 0 ? [{
        '@type': 'Question',
        name: `What is the average tuition fee at UC Davis?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at UC Davis is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * RATE_TO_INR.USD / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      }] : []),
      {
        '@type': 'Question',
        name: `What intake options does UC Davis offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `UC Davis offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to UC Davis?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to UC Davis in USA through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `UC Davis — Courses for International Students`,
    description: `${courses.length} programs at UC Davis, USA. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'UC Davis' },
  ...
  return (
    <>
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">UC Davis — All Courses & Programs 2026</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-3">
          {courses.map(c => (
            <Link key={c.id} href={`/universities/uc-davis/courses/${c.slug}`}
