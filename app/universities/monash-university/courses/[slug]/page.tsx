...
export async function generateStaticParams() {
  return (monashCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  return buildMetadata({
    title: `${course.name} | Monash – Fees, IELTS & Intake 2026`,
    description: `${course.name} at Monash University. Annual fee A$${fee.toLocaleString()} (${course.durationYears} year${course.durationYears !== 1 ? 's' : ''}). IELTS ${course.ieltsMin}+. Intake: ${course.intakeMonths.join(' & ')}. Free guidance from Jaivik Overseas Consultants.`,
