...
import { monashCourses } from '@/data/monash-courses';
import LeadForm from '@/components/LeadForm';
import JsonLd from '@/components/JsonLd';

export const metadata: Metadata = buildMetadata({
  title: 'Monash University International Courses – All Programs, Fees & IELTS 2026',
  description: `Monash University – ${(monashCourses as unknown as any[]).length} courses for international students. Free guidance from Jaivik Overseas Consultants.`,
  path: '/universities/monash-university/courses',
  keywords: ['Monash courses', 'Monash University international', 'study in Australia'],
});
...
export default function CoursesPage() {
  const courses = monashCourses as unknown as any[];
  ...
  const avgFee = pgCourses.length
    ? Math.round(pgCourses.reduce((s: number, c: any) => s + (c.annualAUD || c.annualUSD || 0), 0) / pgCourses.length)
    : Math.round(courses.reduce((s: number, c: any) => s + (c.annualAUD || c.annualUSD || 0), 0) / (totalCourses || 1));
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does Monash University offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Monash University offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at Monash University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at Monash University is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the average tuition fee at Monash University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at Monash University is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * 84 / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      },
      {
        '@type': 'Question',
        name: `What intake options does Monash University offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Monash University offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to Monash University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to Monash University in Australia through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `Monash University — Courses for International Students`,
    description: `${courses.length} programs at Monash University, Australia. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'Monash University' },
  ...
  const feeINRLakh = (avgFee * 54.5 / 100000).toFixed(1);
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-3">Monash University</h1>
          <p className="text-blue-100 text-lg mb-6">
            {totalCourses} Programs for International Students · Melbourne, Australia · IELTS 6.0–7.0+
          </p>
          <div className="flex flex-wrap gap-4">
            ...
            <div className="bg-white/10 rounded-xl px-5 py-3 text-center">
              <div className="text-2xl font-bold">A${avgFee.toLocaleString()}</div>
          ...
          <div key={level} className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {levelCourses.map((course: any) => {
                const fee = course.annualAUD || course.annualUSD || 0;
                const feeStr = fee > 0 ? 'A$' + fee.toLocaleString() + '/yr' : 'Low fees';
                return (
                  <Link
                    key={course.id}
                    href={`/universities/monash-university/courses/${course.slug}`}
      ...
      <section className="bg-brand-50 py-12 px-4">
        <div className="max-w-2xl mx-auto">
          <h2 className="text-2xl font-bold text-center text-brand-900 mb-2">Apply to Monash</h2>
