...
import { heidelbergCourses } from '@/data/heidelberg-courses';
import LeadForm from '@/components/LeadForm';
import JsonLd from '@/components/JsonLd';

export const metadata: Metadata = buildMetadata({
  title: 'Heidelberg University International Courses – Programs, Fees & IELTS 2026',
  description: `Heidelberg University — ${(heidelbergCourses as unknown as any[]).length} courses for international students. IELTS 6.5+. October & April intakes. Free admission guidance from Jaivik Overseas Consultants.`,
  path: '/universities/heidelberg-university/courses',
  keywords: ['Heidelberg courses', 'Heidelberg University international', 'study in Germany', 'Germany university'],
});
...
export default function CoursesPage() {
  const courses = heidelbergCourses as unknown as any[];
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does Heidelberg University offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Heidelberg University offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at Heidelberg University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at Heidelberg University is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the average tuition fee at Heidelberg University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at Heidelberg University is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * 84 / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      },
      {
        '@type': 'Question',
        name: `What intake options does Heidelberg University offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Heidelberg University offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to Heidelberg University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to Heidelberg University in Germany through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `Heidelberg University — Courses for International Students`,
    description: `${courses.length} programs at Heidelberg University, Germany. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'Heidelberg University' },
  ...
  const schema = {
    ...
    name: 'Heidelberg University', sameAs: 'https://www.uni-heidelberg.de/en/',
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇩🇪 Heidelberg, Germany · #87 QS World Ranking
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">
                Heidelberg University — International Courses
              </h1>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { label: 'QS Ranking', value: '#87 QS' },
                  { label: 'Avg PG Fee', value: `€${Math.round(avgFee/1000)}K` },
                  { label: 'Campus', value: 'Heidelberg' },
                ].map(s => (
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="heidelberg-courses-index" defaultCountry="Germany" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div key={level}>
              ...
              <div className="space-y-3">
                {(lvC as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((c: any) => (
                  <Link key={c.slug} href={`/universities/heidelberg-university/courses/${c.slug}`}
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="heidelberg-courses-sidebar" defaultCountry="Germany" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Quick Facts — Heidelberg</h3>
              {[
                ['Established', '1386'],
