...
import { rmitsgCourses } from '@/data/rmitsg-courses';
import { buildMetadata } from '@/lib/seo';
import JsonLd from '@/components/JsonLd';

export const metadata: Metadata = buildMetadata({
  title: 'RMIT Online Singapore Courses & Programs 2026 – Fees, IELTS & Intakes',
  description: '39 programs at RMIT Online Singapore for Indian students. Tuition, IELTS requirements, intake dates. Free counselling by Jaivik Overseas.',
  path: '/universities/rmit-singapore/courses',
});

const levels = ['All', 'Bachelor', 'Master', 'Doctoral', 'Diploma'];

export default function RmitsgCoursesPage() {
  const courses = rmitsgCourses;
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does RMIT Online Singapore offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `RMIT Online Singapore offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at RMIT Online Singapore?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at RMIT Online Singapore is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the average tuition fee at RMIT Online Singapore?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at RMIT Online Singapore is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * 84 / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      },
      {
        '@type': 'Question',
        name: `What intake options does RMIT Online Singapore offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `RMIT Online Singapore offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to RMIT Online Singapore?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to RMIT Online Singapore in Singapore through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `RMIT Online Singapore — Courses for International Students`,
    description: `${courses.length} programs at RMIT Online Singapore, Singapore. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'RMIT Online Singapore' },
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-6">
        ...
        <Link href="/universities/rmit-singapore" className="hover:text-brand-700">RMIT Online Singapore</Link> /
        <span className="text-gray-800 font-medium">Courses</span>
      </div>

      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">RMIT Online Singapore — All Courses & Programs 2026</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-3">
          {courses.map(c => (
            <Link key={c.id} href={`/universities/rmit-singapore/courses/${c.slug}`}
