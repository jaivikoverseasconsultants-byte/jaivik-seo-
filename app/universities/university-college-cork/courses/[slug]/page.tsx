...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `${course.name} at University College Cork — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at University College Cork, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/university-college-cork/courses/${slug}`,
    keywords: [course.name, 'UCC', 'University College Cork', 'study in Ireland', course.level],
    noIndex: true
  });
}
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-brand-700 text-white rounded-2xl p-6">
            ...
            <Link href="/universities/university-college-cork/courses" className="flex items-center gap-2 text-brand-700 hover:text-brand-900 font-medium text-sm">← All UCC Courses</Link>
          </div>
        </div>
      </section>
