...
import { uccCourses } from '@/data/ucc-courses';
...
const UNIVERSITY_SLUG = 'university-college-cork';

export const metadata: Metadata = buildMetadata({
  title: 'University College Cork Courses — Fees & IELTS 2026',
  description: `University College Cork – ${(uccCourses as unknown as any[]).length} courses for international students. Free guidance from Jaivik Overseas Consultants.`,
  path: '/universities/university-college-cork/courses',
  keywords: ['DCU courses', 'University College Cork international', 'study in Ireland'],
});
...
export default function CoursesPage() {
  const courses = uccCourses as unknown as any[];
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does University College Cork offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `University College Cork offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at University College Cork?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at University College Cork is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      ...(_avgFeeUSD > 0 ? [{
        '@type': 'Question',
        name: `What is the average tuition fee at University College Cork?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at University College Cork is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * RATE_TO_INR.USD / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      }] : []),
      {
        '@type': 'Question',
        name: `What intake options does University College Cork offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `University College Cork offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to University College Cork?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to University College Cork in Ireland through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `University College Cork — Courses for International Students`,
    description: `${courses.length} programs at University College Cork, Ireland. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'University College Cork' },
  ...
  const schema = {
    ...
    name: 'University College Cork', sameAs: 'https://www.ucc.ie', url: 'https://www.ucc.ie',
  };
  ...
  return (
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-college-cork" className="hover:text-white">DCU</Link> /
            <span>Courses</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-bold mb-3">University College Cork</h1>
          <p className="text-blue-100 text-lg mb-6">{totalCourses} Programs · Cork, Ireland · IELTS 6.5–7.0+</p>
          ...
          <div key={level} className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {levelCourses.map((course: any) => {
                ...
                return (
                  <Link key={course.id} href={`/universities/university-college-cork/courses/${course.slug}`}
      ...
      <section className="bg-brand-50 py-12 px-4"><div className="max-w-2xl mx-auto"><h2 className="text-2xl font-bold text-center text-brand-900 mb-2">Apply to DCU</h2><p className="text-center text-gray-600 mb-6">Get free expert guidance from Jaivik Overseas Consultants</p><LeadForm /></div></section>
    </>
  );
}
