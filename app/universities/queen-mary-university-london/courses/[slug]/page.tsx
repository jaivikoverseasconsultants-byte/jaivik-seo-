...
import { qmulCourses, getQmulCourseBySlug } from '@/data/qmul-courses';
...
export async function generateStaticParams() {
  return (qmulCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getQmulCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Queen Mary University of London — ${titleFeeFragment(course as any, course.annualINR)}IELTS & Requirements for Indian Students`,
    description: `${course.name} at Queen Mary University of London, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)} IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/queen-mary-university-london/courses/${slug}`,
    keywords: [course.name, 'QMUL', 'Queen Mary University of London', 'study in UK', course.level],
  });
}
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/queen-mary-university-london" className="hover:text-white">QMUL</Link> /
            <Link href="/universities/queen-mary-university-london/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at Queen Mary University of London — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`qmul-course-${slug}`} defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            {entryRequirementsVaryByCourse(UNIVERSITY_SLUG) ? (
              ...
              <p className="text-sm text-gray-600">
                Queen Mary University London publishes one English language requirement across its courses
          ...
          <CourseRichContent course={course as any} universityName="Queen Mary University of London" universitySlug="queen-mary-university-london" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`qmul-course-${slug}-sidebar`} defaultCountry="UK" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/queen-mary-university-london/courses" className="block text-sm text-brand-700 hover:underline">
                  All QMUL Courses →
                </Link>
