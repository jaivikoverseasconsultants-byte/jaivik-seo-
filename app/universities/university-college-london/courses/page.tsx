...
import { uclCourses } from '@/data/ucl-courses';
...
const levelOrder = ["Undergraduate", "Postgraduate", "Masters", "PhD"];
...
export default function CoursesPage() {
  const courses = uclCourses as any[];
  const groups = groupByLevel(courses);
  const total = courses.length;
  const pgC = courses.filter((c: any) => c.studyLevel !== 'Undergraduate');
  const avgFee = pgC.length
    ? Math.round(pgC.reduce((s: number, c: any) => s + c.annualGBP, 0) / pgC.length)
    : Math.round(courses.reduce((s: number, c: any) => s + c.annualGBP, 0) / (total || 1));
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does UCL offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `UCL offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at UCL?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at UCL is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the average tuition fee at UCL?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at UCL is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * 84 / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      },
      {
        '@type': 'Question',
        name: `What intake options does UCL offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `UCL offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to UCL?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to UCL in United Kingdom through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `UCL — Courses for International Students`,
    description: `${courses.length} programs at UCL, United Kingdom. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'UCL' },
  ...
  const schema = {
    ...
    name: 'University College London', sameAs: 'https://www.ucl.ac.uk/',
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇬🇧 London, England · #9 QS World Ranking
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">
                UCL — International Degree Programs 2026
              </h1>
              <p className="text-blue-200 text-lg mb-5">
                {total} programs · Avg £{Math.round(avgFee / 1000)}K PG fee · IELTS 7.0+ · September intake
              </p>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="ucl-courses-index" defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div key={level}>
              ...
              <div className="space-y-3">
                {(lvC as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((c: any) => (
                  <Link key={c.slug} href={`/universities/university-college-london/courses/${c.slug}`}
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="ucl-courses-sidebar" defaultCountry="UK" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Quick Facts — UCL</h3>
              {[
                ['Established', '1826'],
                ['Location', 'London, England'],
                ['IELTS Min', '6.5 UG / 7.0 PG'],
                ['Intake', 'September'],
                ['Post-Study', '2-yr Graduate Route Visa'],
                ['Indian Students', '2,500+ per year'],
              ].map(([k, v]) => (
            ...
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Other UK Universities</h3>
              <div className="space-y-2">
                ...
                <Link href="/universities/country/uk" className="block text-sm text-brand-700 hover:underline">All UK Universities →</Link>
              </div>
            </div>
          </div>
        </div>
