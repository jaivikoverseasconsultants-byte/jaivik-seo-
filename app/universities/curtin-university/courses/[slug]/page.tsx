...
export async function generateStaticParams() {
  return (curtinCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getCurtinCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Curtin University — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at Curtin University, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/curtin-university/courses/${slug}`,
    keywords: [course.name, 'Curtin', 'Curtin University', 'study in Australia', course.level],
    noIndex: true
  });
}
...
export default async function CoursePage(
  ...
  const schema = {
    ...
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'Curtin University', sameAs: 'https://www.curtin.edu.au' },
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/curtin-university" className="hover:text-white">Curtin</Link> /
            <Link href="/universities/curtin-university/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`curtin-course-${slug}`} defaultCountry="Australia" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <CourseRichContent course={course as any} universityName="Curtin University" universitySlug="curtin-university" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`curtin-course-${slug}-sidebar`} defaultCountry="Australia" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/curtin-university/courses" className="block text-sm text-brand-700 hover:underline">All Curtin Courses →</Link>
