...
import { lboroCourses } from '@/data/lboro-courses';
...
const UNIVERSITY_SLUG = 'loughborough-university';
...
export default function CoursesPage() {
  const courses = lboroCourses as unknown as any[];
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does Loughborough University offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Loughborough University offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at Loughborough University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at Loughborough University is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      ...(_avgFeeUSD > 0 ? [{
        '@type': 'Question',
        name: `What is the average tuition fee at Loughborough University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at Loughborough University is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * RATE_TO_INR.USD / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      }] : []),
      {
        '@type': 'Question',
        name: `What intake options does Loughborough University offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Loughborough University offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to Loughborough University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to Loughborough University in United Kingdom through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `Loughborough University — Courses for International Students`,
    description: `${courses.length} programs at Loughborough University, United Kingdom. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'Loughborough University' },
  ...
  const schema = {
    ...
    sameAs: 'https://www.lboro.ac.uk',
    address: { '@type': 'PostalAddress', addressLocality: 'Loughborough', addressRegion: 'England', addressCountry: 'GB' },
  };
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <span className="text-white">Loughborough</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇬🇧 Loughborough, United Kingdom · #226 QS World Ranking
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">
                Loughborough University — International Courses
              </h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="lboro-courses-index" defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div key={level}>
              ...
              <div className="space-y-3">
                {(lvCourses as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((c: any) => (
                  <Link key={c.slug} href={`/universities/loughborough-university/courses/${c.slug}`}
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="lboro-courses-sidebar" defaultCountry="UK" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Quick Facts — Loughborough</h3>
              {[
                ['Established', '1909'],
