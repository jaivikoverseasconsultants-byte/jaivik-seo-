...
import { otagoCourses, getOtagoCourseBySlug } from '@/data/otago-courses';
...
export async function generateMetadata(
  ...
  return buildMetadata({
    ...
    description: `${course.name} at University of Otago, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
...
export default async function CoursePage(
  ...
  const schema = {
    ...
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'University of Otago', sameAs: 'https://www.otago.ac.nz' },
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-otago" className="hover:text-white">Otago</Link> /
            <Link href="/universities/university-of-otago/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{(course as any).withdrawn ? `${course.name} — No Longer Offered` : <>{course.name} at University of Otago — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</>}</h1>
              <p className="text-blue-200 text-lg mb-5">{course.studyLevel} · {course.duration} · {course.campus}</p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Annual Fee (NZD)', value: feeDisplay(course as any, course.annualNZD, 'NZD') },
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`otago-course-${slug}`} defaultCountry="New Zealand" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        {(course as any).withdrawn && (
          <div className="lg:col-span-2 bg-amber-50 border border-amber-300 rounded-2xl p-6" role="note">
            <h2 className="text-lg font-bold text-amber-900 mb-2">
              ⚠️ This course is no longer offered by the University of Otago
            </h2>
              ...
              <>
                ...
                <ul className="space-y-1.5">
                  {((course as any).alternatives as { name: string; slug: string }[]).map(alt => (
                    <li key={alt.slug}>
                      <Link
                        href={`/universities/university-of-otago/courses/${alt.slug}`}
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                { label: 'Total Course Fee', value: feeDisplay(course as any, course.totalNZD, 'NZD') },
              ].map(f => (
            ...
            {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0 && (entryRequirementsVaryByCourse(UNIVERSITY_SLUG) || !!(course as any).englishVerified) ? (
              ...
              <p className="text-sm text-gray-600">
                {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0
                  ? "University Of Otago publishes one English language requirement across its courses rather than a per-course score."
                  : englishOnRequestNote("University Of Otago")}{' '}
          ...
          <CourseRichContent course={course as any} universityName="University of Otago" universitySlug="university-of-otago" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`otago-course-${slug}-sidebar`} defaultCountry="New Zealand" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/university-of-otago/courses" className="block text-sm text-brand-700 hover:underline">All Otago Courses →</Link>
