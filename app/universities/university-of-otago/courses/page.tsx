...
import { otagoCourses } from '@/data/otago-courses';
...
export default function CoursesPage() {
  const courses = otagoCourses as unknown as any[];
  const groups  = groupByLevel(courses);
  const total   = courses.length;
  const pgC     = courses.filter((c: any) => c.studyLevel === 'Masters' || c.studyLevel === 'Postgraduate');
  const avgFee = verifiedAvgFee(pgC.length ? (pgC as any[]) : (courses as any[]), 'annualNZD');
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does University of Otago offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `University of Otago offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at University of Otago?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at University of Otago is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      ...(_avgFeeUSD > 0 ? [{
        '@type': 'Question',
        name: `What is the average tuition fee at University of Otago?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at University of Otago is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * RATE_TO_INR.USD / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      }] : []),
      {
        '@type': 'Question',
        name: `What intake options does University of Otago offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `University of Otago offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to University of Otago?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to University of Otago in New Zealand through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `University of Otago — Courses for International Students`,
    description: `${courses.length} programs at University of Otago, New Zealand. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'University of Otago' },
  ...
  const schema = {
    ...
    name: 'University of Otago', sameAs: 'https://www.otago.ac.nz',
    address: { '@type': 'PostalAddress', addressLocality: 'Dunedin', addressRegion: 'Otago', addressCountry: 'NZ' },
  };
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <span className="text-white">Otago</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇳🇿 Dunedin, New Zealand · #207 QS World Ranking
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">
                University of Otago — International Courses
              </h1>
              <p className="text-blue-200 text-lg mb-5">
                {total} programs · {avgFee > 0 ? `Avg NZ$${avgFee.toLocaleString()}/yr` : 'Fees on request'} · IELTS 6+ · February & July intakes
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Total Courses', value: total },
                  { label: 'QS Ranking', value: '#207 QS' },
                  { label: 'Avg PG Fee', value: avgFee > 0 ? `NZ$${Math.round(avgFee/1000)}K` : 'On request' },
                  { label: 'Campus', value: 'Dunedin' },
                ].map(s => (
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="otago-courses-index" defaultCountry="New Zealand" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div key={level}>
              ...
              <div className="space-y-3">
                {(lvC as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((c: any) => (
                  <Link key={c.slug} href={`/universities/university-of-otago/courses/${c.slug}`}
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="otago-courses-sidebar" defaultCountry="New Zealand" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Quick Facts — Otago</h3>
              {[
                ['Established', '1869'],
