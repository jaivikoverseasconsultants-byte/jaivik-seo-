...
import { karolinskaInstitutetCourses, getKarolinskaInstitutetCourseBySlug } from '@/data/karolinska-institutet-courses';
import { buildMetadata } from '@/lib/seo';
import LeadForm from '@/components/LeadForm';
import JsonLd from '@/components/JsonLd';

export function generateStaticParams() {
  return karolinskaInstitutetCourses.map(c => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  return buildMetadata({
    ...
    description: `${course.name} at Karolinska Institutet: ${course.duration}, SEK ${course.annualSEK.toLocaleString()}/SEK/yr. IELTS ${course.ieltsMin}+. Intake: ${course.intakeMonths.join(' & ')}. Free guidance from Jaivik Overseas.`,
...
export default async function CourseDetailPage(
  ...
  const schema = {
    ...
    '@type': 'Course',
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'Karolinska Institutet', sameAs: 'https://ki.se' },
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/karolinska-institutet/courses" className="hover:text-white">KI</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="karolinska-institutet-${slug}" defaultCountry="Sweden" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div>
          <div className="sticky top-20">
            <LeadForm source="karolinska-institutet-detail-sidebar" defaultCountry="Sweden" />
          </div>
        </div>
      </div>
    </>
