...
import { nyit_vancouverCourses } from '@/data/nyit-vancouver-courses';
import LeadForm from '@/components/LeadForm';
import JsonLd from '@/components/JsonLd';

export const metadata: Metadata = buildMetadata({
  title: 'NYIT International Courses – All Programs, Fees & IELTS 2026',
  description: `NYIT Vancouver — ${(nyit_vancouverCourses as unknown as any[]).length} courses for international students. PGWP Eligible · IELTS 6+. September & January intakes. Free admission guidance from Jaivik Overseas Consultants.`,
  path: '/universities/nyit-vancouver/courses',
  keywords: ['NYIT courses', 'NYIT Vancouver international', 'NYIT fees', 'study in Canada', 'Canada university', 'PGWP'],
});
...
export default function CoursesPage() {
  const courses = nyit_vancouverCourses as unknown as any[];
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does NYIT Vancouver offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `NYIT Vancouver offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at NYIT Vancouver?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at NYIT Vancouver is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the average tuition fee at NYIT Vancouver?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at NYIT Vancouver is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * 84 / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      },
      {
        '@type': 'Question',
        name: `What intake options does NYIT Vancouver offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `NYIT Vancouver offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to NYIT Vancouver?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to NYIT Vancouver in Canada through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `NYIT Vancouver — Courses for International Students`,
    description: `${courses.length} programs at NYIT Vancouver, Canada. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'NYIT Vancouver' },
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <span className="text-white">NYIT</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3">
                NYIT Vancouver — International Courses
              </h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="nyit-vancouver-courses-index" defaultCountry="Canada" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div key={level}>
              ...
              <div className="space-y-3">
                {(lvCourses as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((c: any) => (
                  <Link key={c.slug} href={`/universities/nyit-vancouver/courses/${c.slug}`}
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="nyit-vancouver-courses-sidebar" defaultCountry="Canada" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Quick Facts — NYIT</h3>
              {[
                ['Established', '1955'],
