...
import { rwthCourses, getRwthCourseBySlug } from '@/data/rwth-courses';
...
export async function generateStaticParams() {
  return (rwthCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getRwthCourseBySlug(slug);
  if (!course) return {};
  const fee = (course as any).annualEUR || (course as any).annualUSD || 0;
  return buildMetadata({
    title: `${course.name} | RWTH Aachen – Fees, IELTS & Intake 2026`,
    description: `${course.name} at RWTH Aachen University. Annual fee €${fee.toLocaleString()} (${course.durationYears} year${course.durationYears !== 1 ? 's' : ''}). IELTS ${course.ieltsMin}+. Intake: ${course.intakeMonths.join(' & ')}. Free guidance from Jaivik Overseas Consultants.`,
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/rwth-aachen-university/courses" className="hover:text-white">Courses</Link> /
          ...
          <p className="text-blue-100 text-lg">
            RWTH Aachen University · Aachen, Germany · {course.duration}
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                  ...
                  { label: 'Campus', value: course.campus?.split(',')[0] || 'Aachen', sub: 'Germany' },
                ].map(({ label, value, sub }) => (
            ...
            {/* Why RWTH Aachen */}
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-4">Why Choose RWTH Aachen University?</h2>
          ...
          <CourseRichContent course={course as any} universityName="RWTH Aachen University" universitySlug="rwth-aachen-university" />
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">Aachen</span>
                </div>
              </div>
            </div>

            <Link href="/universities/rwth-aachen-university/courses" className="flex items-center gap-2 text-brand-700 hover:text-brand-900 font-medium text-sm">
              ← All RWTH Aachen Courses
            </Link>
          </div>
        </div>
      </section>
