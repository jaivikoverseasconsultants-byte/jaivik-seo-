...
import { centennialCourses, getCentennialCoursesBySlug } from '@/data/centennial-courses';
...
export async function generateStaticParams() {
  return (centennialCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getCentennialCoursesBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Centennial College — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at Centennial College, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/centennial-college/courses/${slug}`,
    keywords: [course.name, 'Centennial', 'Centennial College', 'study in Canada', course.level, 'PGWP'],
    noIndex: true
  });
}
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/centennial-college" className="hover:text-white">Centennial</Link> /
            <Link href="/universities/centennial-college/courses" className="hover:text-white">Courses</Link> /
            <span className="text-white">{course.name}</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇨🇦 Centennial College · Toronto, Canada · PGWP Eligible
              </div>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`centennial-course-${slug}`} defaultCountry="Canada" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <CourseRichContent course={course as any} universityName="Centennial College" universitySlug="centennial-college" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`centennial-course-${slug}-sidebar`} defaultCountry="Canada" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/centennial-college/courses" className="block text-sm text-brand-700 hover:underline">
                  All Centennial Courses →
                </Link>
