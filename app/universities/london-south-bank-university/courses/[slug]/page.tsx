...
export async function generateMetadata(
  ...
  return buildMetadata({
    title: `${course.name} at London South Bank University — ${titleFeeFragment(course as any, course.annualINR)}IELTS & Requirements for Indian Students`,
    description: `${course.name} at London South Bank University, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)} IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{(course as any).withdrawn ? `${course.name} — No Longer Offered` : <>{course.name} at London South Bank University — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</>}</h1>
        ...
        {(course as any).withdrawn && (
          <div className="lg:col-span-2 bg-amber-50 border border-amber-300 rounded-2xl p-6" role="note">
            <h2 className="text-lg font-bold text-amber-900 mb-2">
              ⚠️ This course is no longer offered by London South Bank University
            </h2>
              ...
              <>
                ...
                <ul className="space-y-1.5">
                  {((course as any).alternatives as { name: string; slug: string }[]).map(alt => (
                    <li key={alt.slug}>
                      <Link
                        href={`/universities/london-south-bank-university/courses/${alt.slug}`}
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            {entryRequirementsVaryByCourse(UNIVERSITY_SLUG) ? (
              ...
              <p className="text-sm text-gray-600">
                London South Bank University publishes one English language requirement across its courses
