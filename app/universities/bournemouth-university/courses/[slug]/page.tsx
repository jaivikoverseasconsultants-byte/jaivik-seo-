...
import { bmouthCourses, getBmouthCourseBySlug } from '@/data/bmouth-courses';
...
export function generateStaticParams() {
  return bmouthCourses.map(c => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getBmouthCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Bournemouth University`,
    description: `${course.name} at Bournemouth University, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/bournemouth-university/courses/${slug}`,
    keywords: [course.name, 'Bournemouth', 'Bournemouth University', 'study in UK', course.level],
  });
}
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/bournemouth-university" className="hover:text-white">Bournemouth</Link> /
            <Link href="/universities/bournemouth-university/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{(course as any).withdrawn ? `${course.name} — No Longer Offered` : <>{course.name} at Bournemouth University — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</>}</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`bmouth-course-${slug}`} defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        {(course as any).withdrawn && (
          <div className="lg:col-span-2 bg-amber-50 border border-amber-300 rounded-2xl p-6" role="note">
            <h2 className="text-lg font-bold text-amber-900 mb-2">
              ⚠️ This course is no longer offered by Bournemouth University
            </h2>
              ...
              <>
                ...
                <ul className="space-y-1.5">
                  {((course as any).alternatives as { name: string; slug: string }[]).map(alt => (
                    <li key={alt.slug}>
                      <Link
                        href={`/universities/bournemouth-university/courses/${alt.slug}`}
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {entryRequirementsVaryByCourse(UNIVERSITY_SLUG) && publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0 ? (
              ...
              <p className="text-sm text-gray-600">
                {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0
                  ? "Bournemouth University publishes one English language requirement across its courses rather than a per-course score."
                  : englishOnRequestNote("Bournemouth University")}{' '}
          ...
          <CourseRichContent course={course as any} universityName="Bournemouth University" universitySlug="bournemouth-university" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`bmouth-course-${slug}-sidebar`} defaultCountry="UK" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/bournemouth-university/courses" className="block text-sm text-brand-700 hover:underline">
                  All Bournemouth Courses →
                </Link>
