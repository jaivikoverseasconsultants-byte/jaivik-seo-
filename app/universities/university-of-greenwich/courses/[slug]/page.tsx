...
import { greenwichCourses, getGreenwichCourseBySlug } from '@/data/greenwich-courses';
...
const UNIVERSITY_SLUG = 'university-of-greenwich';

export async function generateStaticParams() {
  return greenwichCourses.map((c) => ({ slug: c.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `${course.name} at University of Greenwich`,
    description: `${course.name} at University of Greenwich, London${feeSentenceINR(course as any, course.annualINR)}${course.ieltsMin > 0 ? `${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''},` : ''} Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
...
export default async function CoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = { '@context': 'https://schema.org', '@type': 'Course', name: course.name, provider: { '@type': 'CollegeOrUniversity', name: 'University of Greenwich', sameAs: 'https://www.gre.ac.uk' }, courseMode: 'full-time', educationalLevel: course.studyLevel };
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/university-of-greenwich" className="hover:text-white">Greenwich</Link> /
            <Link href="/universities/university-of-greenwich/courses" className="hover:text-white">Courses</Link> /
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at University of Greenwich — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
          <p className="text-blue-100 text-lg">University of Greenwich · London, UK · {course.duration}</p>
        </div>
      </section>
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <CourseKeyFacts course={course as any} universityName="University of Greenwich" universitySlug="university-of-greenwich" />
          </div>
          <div className="space-y-6">
            <CourseRichContent course={course as any} universityName="University of Greenwich" universitySlug="university-of-greenwich" />
            ...
            <Link href="/universities/university-of-greenwich/courses" className="flex items-center gap-2 text-brand-700 hover:text-brand-900 font-medium text-sm">← All Greenwich Courses</Link>
          </div>
        </div>
      </section>
