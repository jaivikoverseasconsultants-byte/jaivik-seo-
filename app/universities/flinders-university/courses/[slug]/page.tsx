...
import { flindersCourses, getFlindersCoursesBySlug } from '@/data/flinders-courses';
...
const UNIVERSITY_SLUG = 'flinders-university';
export async function generateStaticParams() {
  return (flindersCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getFlindersCoursesBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Flinders University`,
    description: `${course.name} at Flinders University, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/flinders-university/courses/${slug}`,
    keywords: [course.name, 'Flinders', 'Flinders University', 'study in Australia', course.level],
  });
}
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/flinders-university" className="hover:text-white">Flinders</Link> /
            <span className="text-white">{course.name}</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇦🇺 Flinders University · {course.campus}
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at Flinders University — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`flinders-course-${slug}`} defaultCountry="Australia" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
            ...
            {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0 && (entryRequirementsVaryByCourse(UNIVERSITY_SLUG) || !!(course as any).englishVerified) ? (
              ...
              <p className="text-sm text-gray-600">
                {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0
                  ? "Flinders University publishes one English language requirement across its courses rather than a per-course score."
                  : englishOnRequestNote("Flinders University")}{' '}
          ...
          <CourseRichContent course={course as any} universityName="Flinders University" universitySlug="flinders-university" />
          <div className="bg-brand-700 rounded-2xl p-6 text-white text-center">
            ...
            <p className="text-blue-200 text-sm mb-4">
              Book a free counselling session. Our advisors specialise in Flinders admissions for Indian students.
            </p>
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`flinders-course-${slug}-sidebar`} defaultCountry="Australia" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/flinders-university/courses" className="block text-sm text-brand-700 hover:underline">
                  All Flinders Courses →
                </Link>
