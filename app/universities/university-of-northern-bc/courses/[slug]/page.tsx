...
import { unbcCourses, getUnbcCoursesBySlug } from '@/data/unbc-courses';
...
export async function generateStaticParams() {
  return (unbcCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  return buildMetadata({
    title: `${course.name} at University of Northern British Columbia — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at University of Northern British Columbia, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-northern-bc" className="hover:text-white">UNBC</Link> /
            <Link href="/universities/university-of-northern-bc/courses" className="hover:text-white">Courses</Link> /
            <span className="text-white">{course.name}</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇨🇦 University of Northern British Columbia · Prince George, Canada · PGWP Eligible
              </div>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`unbc-course-${slug}`} defaultCountry="Canada" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <CourseRichContent course={course as any} universityName="University of Northern British Columbia" universitySlug="university-of-northern-bc" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`unbc-course-${slug}-sidebar`} defaultCountry="Canada" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/university-of-northern-bc/courses" className="block text-sm text-brand-700 hover:underline">
                  All UNBC Courses →
                </Link>
