...
export function generateStaticParams() {
  return vanderbiltCourses.map(c => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const c = vanderbiltCourses.find(x => x.slug === slug);
  if (!c) return {};
  return buildMetadata({
    title: `${c.name} at Vanderbilt University 2026 – Fees, IELTS & Requirements`,
    description: `${c.name} at Vanderbilt University: ${c.duration}, $${(c.annualUSD/1000).toFixed(0)}K USD/yr. IELTS ${c.ieltsMin}+. Apply via Jaivik Overseas Consultants.`,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-6 flex-wrap">
        ...
        <Link href="/universities/vanderbilt-university/courses" className="hover:text-brand-700">Courses</Link> /
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500 text-sm">Vanderbilt University · {c.city}, {c.state}</p>
          ...
          <CourseRichContent course={c as any} universityName="Vanderbilt University" universitySlug="vanderbilt-university" />
          <div className="bg-brand-700 rounded-2xl p-6 text-white text-center">
            <h2 className="text-lg font-bold mb-2">Apply for {c.name} at Vanderbilt University</h2>
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="vanderbilt-university-course" defaultCountry="USA" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">More Programs at Vanderbilt University</h3>
              <div className="space-y-2">
                {vanderbiltCourses.filter(x=>x.slug!==slug).slice(0,6).map(x=>(
                  <Link key={x.id} href={`/universities/vanderbilt-university/courses/${x.slug}`}
                    className="block text-sm text-brand-700 hover:underline">
                    {x.name} →
                  </Link>
                ))}
              </div>
              <Link href="/universities/vanderbilt-university/courses" className="block text-xs text-gray-500 hover:underline mt-3">
                All Vanderbilt University programs →
              </Link>
            </div>
          </div>
        </div>
