...
import { abdnCourses, getAbdnCourseBySlug } from '@/data/abdn-courses';
...
export async function generateStaticParams() {
  return (abdnCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getAbdnCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at University of Aberdeen — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at University of Aberdeen, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/university-of-aberdeen/courses/${slug}`,
    keywords: [course.name, 'Aberdeen', 'University of Aberdeen', 'study in UK', course.level],
    noIndex: true
  });
}
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-aberdeen" className="hover:text-white">Aberdeen</Link> /
            <Link href="/universities/university-of-aberdeen/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`abdn-course-${slug}`} defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <CourseRichContent course={course as any} universityName="University of Aberdeen" universitySlug="university-of-aberdeen" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`abdn-course-${slug}-sidebar`} defaultCountry="UK" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/university-of-aberdeen/courses" className="block text-sm text-brand-700 hover:underline">
                  All Aberdeen Courses →
                </Link>
