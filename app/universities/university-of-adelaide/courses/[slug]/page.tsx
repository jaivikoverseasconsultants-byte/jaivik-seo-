...
export async function generateStaticParams() {
  return (uoaCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  return buildMetadata({
    title: `${course.name} | Adelaide – Fees, IELTS & Intake 2026`,
    description: `${course.name} at University of Adelaide. Annual fee A$${course.annualAUD.toLocaleString()} (${course.durationYears} year${course.durationYears !== 1 ? 's' : ''}). IELTS ${course.ieltsMin}+. Intake: ${course.intakeMonths.join(' & ')}. Free guidance from Jaivik Overseas Consultants.`,
