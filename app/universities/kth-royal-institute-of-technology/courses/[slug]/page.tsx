...
import { kthRoyalInstituteOfTechnologyCourses, getKthRoyalInstituteOfTechnologyCourseBySlug } from '@/data/kth-royal-institute-of-technology-courses';
...
export async function generateStaticParams() {
  return (kthRoyalInstituteOfTechnologyCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getKthRoyalInstituteOfTechnologyCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at KTH Royal Institute of Technology — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at KTH Royal Institute of Technology, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/kth-royal-institute-of-technology/courses/${slug}`,
    keywords: [course.name, 'KTH Royal Institute of Technology', 'study in Sweden', course.level],
  });
}
...
export default async function CourseDetailPage(
  ...
  const schema = {
    ...
    '@type': 'Course',
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'KTH Royal Institute of Technology', sameAs: 'https://www.kth.se' },
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/kth-royal-institute-of-technology/courses" className="hover:text-white">KTH</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="kth-royal-institute-of-technology-${slug}" defaultCountry="Sweden" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div>
          <div className="sticky top-20">
            <LeadForm source="kth-royal-institute-of-technology-detail-sidebar" defaultCountry="Sweden" />
          </div>
        </div>
      </div>
    </>
