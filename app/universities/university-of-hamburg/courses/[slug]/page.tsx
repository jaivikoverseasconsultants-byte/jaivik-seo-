...
import { uhamCourses, getUhamCourseBySlug } from '@/data/uham-courses';
...
const UNIVERSITY_SLUG = 'university-of-hamburg';
export async function generateStaticParams() {
  return (uhamCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getUhamCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at University of Hamburg — ${titleFeeFragment(course as any, course.annualINR)}IELTS & Requirements for Indian Students`,
...
export default async function CoursePage(
  ...
  const schema = {
    ...
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'University of Hamburg', sameAs: 'https://www.uni-hamburg.de/en/' },
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-hamburg" className="hover:text-white">Hamburg</Link> /
            <Link href="/universities/university-of-hamburg/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at University of Hamburg — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`uham-course-${slug}`} defaultCountry="Germany" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'Total Est. Cost', value: (isFeeVerified(course as any) ? `€${(course.totalEUR + course.livingCostEUR * course.durationYears).toLocaleString()}` : 'On request'), hi: true },
                { label: 'In Indian Rupees', value: (isFeeVerified(course as any) ? `₹${((course.totalEUR + course.livingCostEUR * course.durationYears) * 1.08 * 84 / 100000).toFixed(1)} Lakh` : 'On request'), hi: true },
              ].map(r => (
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { label: 'Student Visa', value: 'Germany Student Visa' },
                { label: 'Work Rights (Term)', value: '20 hrs/week' },
                { label: 'Work Rights (Vacation)', value: 'Full-time' },
                ...(isPswEligible(course as any) ? [{ label: 'Post-Study Visa', value: '18-month Job Seeker Visa post-study' }] : []),
              ].map(f => (
                <div key={f.label} className="p-4 bg-gray-50 rounded-xl">
                  <p className="text-xs text-gray-500 font-medium mb-1">{f.label}</p>
                  <p className="text-sm font-semibold text-gray-900">{f.value}</p>
          ...
          <CourseRichContent course={course as any} universityName="University of Hamburg" universitySlug="university-of-hamburg" />
          <div className="bg-brand-700 rounded-2xl p-6 text-white text-center">
            ...
            <p className="text-blue-200 text-sm mb-4">Our Germany admissions advisors give free, personalised guidance to Indian students.</p>
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`uham-course-${slug}-sidebar`} defaultCountry="Germany" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/university-of-hamburg/courses" className="block text-sm text-brand-700 hover:underline">All Hamburg Courses →</Link>
