...
import { sheffieldCourses, getSheffieldCourseBySlug } from '@/data/sheffield-courses';
...
export async function generateStaticParams() {
  return (sheffieldCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  return buildMetadata({
    ...
    description: `${course.name} at University of Sheffield, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-sheffield" className="hover:text-white">Sheffield</Link> /
            <Link href="/universities/university-of-sheffield/courses" className="hover:text-white">Courses</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at University of Sheffield — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`sheffield-course-${slug}`} defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
            ...
            {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0 && (entryRequirementsVaryByCourse(UNIVERSITY_SLUG) || !!(course as any).englishVerified) ? (
              ...
              <p className="text-sm text-gray-600">
                {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0
                  ? "University Of Sheffield publishes one English language requirement across its courses rather than a per-course score."
                  : englishOnRequestNote("University Of Sheffield")}{' '}
          ...
          <CourseRichContent course={course as any} universityName="University of Sheffield" universitySlug="university-of-sheffield" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`sheffield-course-${slug}-sidebar`} defaultCountry="UK" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/university-of-sheffield/courses" className="block text-sm text-brand-700 hover:underline">
                  All Sheffield Courses →
                </Link>
