...
import { kitCourses, getKitCourseBySlug } from '@/data/kit-courses';
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `${course.name} at Karlsruhe Institute of Technology — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at Karlsruhe Institute of Technology, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
...
export default async function CoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = { '@context': 'https://schema.org', '@type': 'Course', name: course.name, provider: { '@type': 'CollegeOrUniversity', name: 'Karlsruhe Institute of Technology', sameAs: 'https://www.kit.edu' }, courseMode: 'full-time', educationalLevel: course.studyLevel };
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/karlsruhe-institute-of-technology/courses" className="hover:text-white">Courses</Link> /
          ...
          <p className="text-blue-100 text-lg">Karlsruhe Institute of Technology · Karlsruhe, Germany · {course.duration}</p>
        </div>
      </section>
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                  ...
                  { label: 'Campus', value: 'Karlsruhe', sub: 'Germany' },
                ].map(({ label, value, sub }) => (
          ...
          <div className="bg-brand-700 text-white rounded-2xl p-6">
            ...
            <Link href="/universities/karlsruhe-institute-of-technology/courses" className="flex items-center gap-2 text-brand-700 hover:text-brand-900 font-medium text-sm">← All KIT Courses</Link>
          </div>
        </div>
      </section>
