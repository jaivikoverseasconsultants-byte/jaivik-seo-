...
function groupByLevel(courses: typeof uclanCourses) {
  const groups: Record<string, typeof uclanCourses> = {};
  courses.forEach((c) => { const lv = c.studyLevel || 'Other'; if (!groups[lv]) groups[lv] = []; groups[lv].push(c); });
  return groups;
}

export default function CoursesPage() {
  const courses = uclanCourses;
  ...
  const orderedGroups = levelOrder.filter(l => groups[l]).map(l => [l, groups[l]] as [string, typeof uclanCourses]);
  const allGroups = [...orderedGroups, ...Object.keys(groups).filter(l => !levelOrder.includes(l)).map(l => [l, groups[l]] as [string, typeof uclanCourses])];

  const schema = { '@context': 'https://schema.org', '@type': 'CollegeOrUniversity', name: 'University of Central Lancashire', sameAs: 'https://www.uclan.ac.uk', url: 'https://www.uclan.ac.uk' };
  const courseListSchema = {
    ...
    itemListElement: courses.slice(0, 5).map((c, i) => ({
      ...
      item: { '@type': 'Course', name: c.name, provider: { '@type': 'CollegeOrUniversity', name: 'University of Central Lancashire' }, educationalLevel: c.studyLevel },
    })),
  };
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-3">University of Central Lancashire</h1>
          <p className="text-blue-100 text-lg mb-6">{courses.length} Programs · Preston, UK</p>
          ...
          <div key={level} className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {levelCourses.map((course) => {
                ...
                return (
                  <Link key={course.id} href={`/universities/university-of-central-lancashire/courses/${course.slug}`}
      ...
      <section className="bg-brand-50 py-12 px-4"><div className="max-w-2xl mx-auto"><h2 className="text-2xl font-bold text-center text-brand-900 mb-2">Apply to UCLan</h2><p className="text-center text-gray-600 mb-6">Get free expert guidance from Jaivik Overseas Consultants</p><LeadForm /></div></section>
    </>
  );
}
