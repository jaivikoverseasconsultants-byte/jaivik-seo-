...
import { imperialCourses } from '@/data/imperial-courses';
...
function groupByLevel(courses: typeof imperialCourses) {
  const groups: Record<string, typeof imperialCourses> = {};
  courses.forEach((c) => { const lv = c.studyLevel || 'Other'; if (!groups[lv]) groups[lv] = []; groups[lv].push(c); });
  return groups;
}

export default function CoursesPage() {
  const courses = imperialCourses;
  ...
  const orderedGroups = levelOrder.filter(l => groups[l]).map(l => [l, groups[l]] as [string, typeof imperialCourses]);
  const allGroups = [...orderedGroups, ...Object.keys(groups).filter(l => !levelOrder.includes(l)).map(l => [l, groups[l]] as [string, typeof imperialCourses])];

  const schema = { '@context': 'https://schema.org', '@type': 'CollegeOrUniversity', name: 'Imperial College London', sameAs: 'https://www.imperial.ac.uk', url: 'https://www.imperial.ac.uk' };
  const courseListSchema = {
    '@context': 'https://schema.org', '@type': 'ItemList',
    name: 'Imperial College London — Courses for International Students',
    numberOfItems: courses.length,
    itemListElement: courses.slice(0, 5).map((c, i) => ({
      '@type': 'ListItem', position: i + 1,
      item: { '@type': 'Course', name: c.name, provider: { '@type': 'CollegeOrUniversity', name: 'Imperial College London' }, educationalLevel: c.studyLevel },
    })),
  };
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-3">Imperial College London</h1>
          ...
          <div key={level} className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {levelCourses.map((course) => {
                ...
                return (
                  <Link key={course.id} href={`/universities/imperial-college-london/courses/${course.slug}`}
      ...
      <section className="bg-brand-50 py-12 px-4"><div className="max-w-2xl mx-auto"><h2 className="text-2xl font-bold text-center text-brand-900 mb-2">Apply to Imperial</h2><p className="text-center text-gray-600 mb-6">Get free expert guidance from Jaivik Overseas Consultants</p><LeadForm /></div></section>
    </>
  );
}
