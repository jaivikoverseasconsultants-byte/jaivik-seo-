...
import { heriotWattUniversityDubaiCourses, getHeriotWattUniversityDubaiCourseBySlug } from '@/data/heriot-watt-university-dubai-courses';
...
export async function generateMetadata(
  ...
  return buildMetadata({
    ...
    description: `${course.name} at Heriot-Watt University Dubai, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
...
export default async function CourseDetailPage(
  ...
  const schema = {
    ...
    '@type': 'Course',
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'Heriot-Watt University Dubai', sameAs: 'https://www.hw.ac.uk/dubai' },
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/heriot-watt-university-dubai/courses" className="hover:text-white">HWU Dubai</Link> /
            <span className="text-white">{course.name}</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇦🇪 Heriot-Watt University Dubai · Dubai, UAE
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at Heriot-Watt University Dubai — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="heriot-watt-university-dubai-${slug}" defaultCountry="UAE" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <CourseRichContent course={course as any} universityName="Heriot-Watt University Dubai" universitySlug="heriot-watt-university-dubai" />
        ...
        <div>
          <div className="sticky top-20">
            <LeadForm source="heriot-watt-university-dubai-detail-sidebar" defaultCountry="UAE" />
          </div>
        </div>
      </div>
    </>
