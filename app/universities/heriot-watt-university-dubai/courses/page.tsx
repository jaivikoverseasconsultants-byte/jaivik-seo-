...
import { heriotWattUniversityDubaiCourses } from '@/data/heriot-watt-university-dubai-courses';
...
const UNIVERSITY_SLUG = 'heriot-watt-university-dubai';
...
export default function CoursesPage() {
  const courses = heriotWattUniversityDubaiCourses as any[];
  const groups = groupByLevel(courses);
  const totalCourses = courses.length;
  const avgFee = verifiedAvgFee(courses as any[], 'annualAED');
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does Heriot-Watt University Dubai offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Heriot-Watt University Dubai offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at Heriot-Watt University Dubai?`,
        acceptedAnswer: {
          '@type': 'Answer',
      ...
      {
        '@type': 'Question',
        name: `What intake options does Heriot-Watt University Dubai offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Heriot-Watt University Dubai offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to Heriot-Watt University Dubai?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to Heriot-Watt University Dubai in UAE through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `Heriot-Watt University Dubai — Courses for International Students`,
    description: `${courses.length} programs at Heriot-Watt University Dubai, UAE. Min IELTS ${_minIelts}+.`,
  ...
  const schema = {
    ...
    sameAs: 'https://www.hw.ac.uk/dubai',
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <span className="text-white">HWU Dubai</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇦🇪 Dubai, UAE · #350 QS World Ranking
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { label: 'Avg Fee', value: avgFee > 0 ? `AED ${Math.round(avgFee / 1000)}K` : 'On request' },
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="heriot-watt-university-dubai-courses" defaultCountry="UAE" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div key={level}>
              ...
              <div className="space-y-3">
                {(lvCourses as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((c: any) => (
                  <Link key={c.slug} href={`/universities/heriot-watt-university-dubai/courses/${c.slug}`}
                    ...
                    <div className="ml-4 text-right flex-shrink-0">
                      <p className="text-sm font-bold text-brand-700">{isFeeVerified(c as any) && Number(c.annualAED) > 0 ? `AED ${c.annualAED.toLocaleString()}/yr` : 'Fee on request'}</p>
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="heriot-watt-university-dubai-sidebar" defaultCountry="UAE" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Quick Facts — HWU Dubai</h3>
              {[
                ['Location', 'Dubai, UAE'],
