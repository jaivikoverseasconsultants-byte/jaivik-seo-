...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `${course.name} | Humboldt Berlin – Fees, IELTS & Intake 2026`,
    description: `${course.name} at Humboldt University of Berlin. Annual fee €${fee.toLocaleString()}. IELTS ${course.ieltsMin}+. Intake: ${course.intakeMonths.join(' & ')}. Free guidance from Jaivik Overseas Consultants.`,
...
export default async function CoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = { '@context': 'https://schema.org', '@type': 'Course', name: course.name, provider: { '@type': 'CollegeOrUniversity', name: 'Humboldt University of Berlin', sameAs: 'https://www.hu-berlin.de' }, courseMode: 'full-time', educationalLevel: course.studyLevel };
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/humboldt-university-berlin/courses" className="hover:text-white">Courses</Link> /
          ...
          <p className="text-blue-100 text-lg">Humboldt University of Berlin · Berlin, Germany · {course.duration}</p>
        </div>
      </section>
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                  ...
                  { label: 'Campus', value: 'Berlin', sub: 'Germany' },
                ].map(({ label, value, sub }) => (
          ...
          <CourseRichContent course={course as any} universityName="Humboldt University of Berlin" universitySlug="humboldt-university-berlin" />
          <div className="bg-brand-700 text-white rounded-2xl p-6">
            ...
            <Link href="/universities/humboldt-university-berlin/courses" className="flex items-center gap-2 text-brand-700 hover:text-brand-900 font-medium text-sm">← All Humboldt Berlin Courses</Link>
          </div>
        </div>
      </section>
