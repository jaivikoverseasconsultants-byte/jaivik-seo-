...
import { sciencesPoParisCourses, getSciencesPoParisCourseBySlug } from '@/data/sciences-po-paris-courses';
...
export async function generateStaticParams() {
  return (sciencesPoParisCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getSciencesPoParisCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Sciences Po Paris — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at Sciences Po Paris, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/sciences-po-paris/courses/${slug}`,
    keywords: [course.name, 'Sciences Po Paris', 'study in France', course.level],
  });
}
...
export default async function CourseDetailPage(
  ...
  const schema = {
    ...
    '@type': 'Course',
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'Sciences Po Paris', sameAs: 'https://www.sciencespo.fr' },
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/sciences-po-paris/courses" className="hover:text-white">Sciences Po</Link> /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="sciences-po-paris-${slug}" defaultCountry="France" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <CourseRichContent course={course as any} universityName="Sciences Po Paris" universitySlug="sciences-po-paris" />
        ...
        <div>
          <div className="sticky top-20">
            <LeadForm source="sciences-po-paris-detail-sidebar" defaultCountry="France" />
          </div>
        </div>
      </div>
    </>
