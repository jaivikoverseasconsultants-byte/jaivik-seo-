...
import { pennstateCourses } from '@/data/pennstate-courses';
import { buildMetadata } from '@/lib/seo';
import JsonLd from '@/components/JsonLd';

export const metadata: Metadata = buildMetadata({
  title: 'Penn State University Courses & Programs 2026 – Fees, IELTS & Intakes',
  description: '39 programs at Penn State University for Indian students. Tuition, IELTS requirements, intake dates. Free counselling by Jaivik Overseas.',
  path: '/universities/penn-state-university/courses',
});

const levels = ['All', 'Bachelor', 'Master', 'Doctoral', 'Diploma'];

export default function PennstateCoursesPage() {
  const courses = pennstateCourses;
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does Penn State University offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Penn State University offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at Penn State University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at Penn State University is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the average tuition fee at Penn State University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at Penn State University is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * 84 / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      },
      {
        '@type': 'Question',
        name: `What intake options does Penn State University offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Penn State University offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to Penn State University?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to Penn State University in USA through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `Penn State University — Courses for International Students`,
    description: `${courses.length} programs at Penn State University, USA. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'Penn State University' },
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-6">
        ...
        <Link href="/universities/penn-state-university" className="hover:text-brand-700">Penn State University</Link> /
        <span className="text-gray-800 font-medium">Courses</span>
      </div>

      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Penn State University — All Courses & Programs 2026</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-3">
          {courses.map(c => (
            <Link key={c.id} href={`/universities/penn-state-university/courses/${c.slug}`}
