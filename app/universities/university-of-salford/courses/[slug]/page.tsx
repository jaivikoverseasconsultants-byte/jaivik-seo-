...
import { salfordW2Courses, getSalfordW2CourseBySlug } from '@/data/salford-w2-courses';
...
export async function generateStaticParams() {
  return salfordW2Courses.map((c) => ({ slug: c.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const course = getSalfordW2CourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at University of Salford`,
    description: `${course.name} at University of Salford, Salford${feeSentenceINR(course as any, course.annualINR)}${course.ieltsMin > 0 ? `${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''},` : ''} Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
...
export default async function CoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = { '@context': 'https://schema.org', '@type': 'Course', name: course.name, provider: { '@type': 'CollegeOrUniversity', name: 'University of Salford', sameAs: 'https://www.salford.ac.uk' }, courseMode: 'full-time', educationalLevel: course.studyLevel };
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/university-of-salford/courses" className="hover:text-white">Courses</Link> /
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at University of Salford — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
          <p className="text-blue-100 text-lg">University of Salford · Salford, UK · {course.duration}</p>
        </div>
      </section>
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                  ...
                  { label: 'Campus', value: 'Salford', sub: 'UK' },
            ...
            <CourseKeyFacts course={course as any} universityName="University of Salford" universitySlug="university-of-salford" />
          </div>
          <div className="space-y-6">
            <CourseRichContent course={course as any} universityName="University of Salford" universitySlug="university-of-salford" />
            ...
            <Link href="/universities/university-of-salford/courses" className="flex items-center gap-2 text-brand-700 hover:text-brand-900 font-medium text-sm">← All Salford Courses</Link>
          </div>
        </div>
      </section>
