...
import { caFoscariUniversityVeniceCourses, getCaFoscariUniversityVeniceCourseBySlug } from '@/data/ca-foscari-university-venice-courses';
...
export async function generateStaticParams() {
  return (caFoscariUniversityVeniceCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getCaFoscariUniversityVeniceCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Ca Foscari University Venice — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at Ca Foscari University Venice, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/ca-foscari-university-venice/courses/${slug}`,
    keywords: [course.name, "Ca' Foscari University of Venice", 'study in Italy', course.level],
  });
}
...
export default async function CourseDetailPage(
  ...
  const schema = {
    ...
    '@type': 'Course',
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: UNI_NAME, sameAs: 'https://www.unive.it' },
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/ca-foscari-university-venice/courses" className="hover:text-white">Ca&apos; Foscari</Link> /
            <span className="text-white">{course.name}</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇮🇹 {UNI_NAME} · Venice, Italy
              </div>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`ca-foscari-${slug}`} defaultCountry="Italy" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div>
          <div className="sticky top-20">
            <LeadForm source="ca-foscari-detail-sidebar" defaultCountry="Italy" />
          </div>
        </div>
      </div>
    </>
