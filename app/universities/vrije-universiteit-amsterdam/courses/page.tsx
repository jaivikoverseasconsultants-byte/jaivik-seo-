...
import { vrijeUniversiteitAmsterdamCourses } from '@/data/vrije-universiteit-amsterdam-courses';
...
export default function CoursesPage() {
  const courses = vrijeUniversiteitAmsterdamCourses as any[];
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does Vrije Universiteit Amsterdam offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Vrije Universiteit Amsterdam offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at Vrije Universiteit Amsterdam?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at Vrije Universiteit Amsterdam is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      ...(_avgFeeUSD > 0 ? [{
        '@type': 'Question',
        name: `What is the average tuition fee at Vrije Universiteit Amsterdam?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at Vrije Universiteit Amsterdam is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * RATE_TO_INR.USD / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      }] : []),
      {
        '@type': 'Question',
        name: `What intake options does Vrije Universiteit Amsterdam offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Vrije Universiteit Amsterdam offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to Vrije Universiteit Amsterdam?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to Vrije Universiteit Amsterdam in Netherlands through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `Vrije Universiteit Amsterdam — Courses for International Students`,
    description: `${courses.length} programs at Vrije Universiteit Amsterdam, Netherlands. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'Vrije Universiteit Amsterdam' },
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <span className="text-white">VU Amsterdam</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇳🇱 Amsterdam, Netherlands · #176 QS World Ranking
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">Vrije Universiteit Amsterdam — International Courses</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="vrije-universiteit-amsterdam-courses" defaultCountry="Netherlands" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div key={level}>
              ...
              <div className="space-y-3">
                {(lvCourses as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((c: any) => (
                  <Link key={c.slug} href={`/universities/vrije-universiteit-amsterdam/courses/${c.slug}`}
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="vrije-universiteit-amsterdam-sidebar" defaultCountry="Netherlands" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Quick Facts — VU Amsterdam</h3>
              {[
