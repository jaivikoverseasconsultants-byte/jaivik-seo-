...
import { ubcCourses, getUbcCoursesBySlug } from '@/data/ubc-courses';
...
export async function generateStaticParams() {
  return (ubcCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getUbcCoursesBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at University of British Columbia — ${titleFeeFragment(course as any, course.annualINR)}IELTS & Requirements for Indian Students`,
    description: `${course.name} at University of British Columbia, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)} IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/university-of-british-columbia/courses/${slug}`,
    keywords: [course.name, 'UBC', 'University of British Columbia', 'study in Canada', course.level, 'PGWP'],
  });
}
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/university-of-british-columbia" className="hover:text-white">UBC</Link> /
            <Link href="/universities/university-of-british-columbia/courses" className="hover:text-white">Courses</Link> /
            <span className="text-white">{course.name}</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇨🇦 University of British Columbia · Vancouver, Canada · PGWP Eligible
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at University of British Columbia — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`ubc-course-${slug}`} defaultCountry="Canada" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            {entryRequirementsVaryByCourse(UNIVERSITY_SLUG) ? (
              ...
              <p className="text-sm text-gray-600">
                University Of British Columbia publishes one English language requirement across its courses
          ...
          <CourseRichContent course={course as any} universityName="University of British Columbia" universitySlug="university-of-british-columbia" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`ubc-course-${slug}-sidebar`} defaultCountry="Canada" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/university-of-british-columbia/courses" className="block text-sm text-brand-700 hover:underline">
                  All UBC Courses →
                </Link>
