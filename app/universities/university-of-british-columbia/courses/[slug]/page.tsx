...
export async function generateMetadata(
  ...
  return buildMetadata({
    title: `${course.name} at University of British Columbia`,
    description: `${course.name} at University of British Columbia, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
            ...
            {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0 && (entryRequirementsVaryByCourse(UNIVERSITY_SLUG) || !!(course as any).englishVerified) ? (
              ...
              <p className="text-sm text-gray-600">
                {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0
                  ? "University Of British Columbia publishes one English language requirement across its courses rather than a per-course score."
                  : englishOnRequestNote("University Of British Columbia")}{' '}
