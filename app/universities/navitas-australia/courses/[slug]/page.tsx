...
import { navitasCourses, getNavitasCourseBySlug } from '@/data/navitas-courses';
...
export async function generateStaticParams() {
  return (navitasCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getNavitasCourseBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at Navitas — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${course.name} at Navitas, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/navitas-australia/courses/${slug}`,
    keywords: [course.name, 'Navitas', 'Navitas', 'study in Australia', course.level],
  });
}
...
export default async function CoursePage(
  ...
  const schema = {
    ...
    name: course.name,
    provider: { '@type': 'CollegeOrUniversity', name: 'Navitas', sameAs: 'https://www.navitas.com/australia' },
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <Link href="/universities/navitas-australia" className="hover:text-white">Navitas</Link> /
            <Link href="/universities/navitas-australia/courses" className="hover:text-white">Courses</Link> /
            <span className="text-white">{course.name}</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇦🇺 Navitas · Perth, Australia
              </div>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`navitas-course-${slug}`} defaultCountry="Australia" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <CourseRichContent course={course as any} universityName="Navitas" universitySlug="navitas-australia" />
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`navitas-course-${slug}-sidebar`} defaultCountry="Australia" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/navitas-australia/courses" className="block text-sm text-brand-700 hover:underline">All Navitas Courses →</Link>
