...
export async function generateStaticParams() {
  return (wpiCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const c = wpiCourses.find(x => x.slug === slug);
  if (!c) return {};
  return buildMetadata({
    title: `${c.name} at Worcester Polytechnic Institute — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${c.name} at Worcester Polytechnic Institute, ${(c as any).city || c.country} costs ₹${(c.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${c.ieltsMin}+, intakes ${c.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/wpi/courses/${slug}`,
  });
}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-6 flex-wrap">
        ...
        <Link href="/universities/wpi/courses" className="hover:text-brand-700">Courses</Link> /
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500 text-sm">Worcester Polytechnic Institute · {c.city}, {c.state}</p>
          ...
          <CourseRichContent course={c as any} universityName="Worcester Polytechnic Institute" universitySlug="wpi" />
          <div className="bg-brand-700 rounded-2xl p-6 text-white text-center">
            <h2 className="text-lg font-bold mb-2">Apply for {c.name} at Worcester Polytechnic Institute</h2>
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="wpi-course" defaultCountry="USA" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">More Programs at Worcester Polytechnic Institute</h3>
              <div className="space-y-2">
                {wpiCourses.filter(x=>x.slug!==slug).slice(0,6).map(x=>(
                  <Link key={x.id} href={`/universities/wpi/courses/${x.slug}`}
                    className="block text-sm text-brand-700 hover:underline">
                    {x.name} →
                  </Link>
                ))}
              </div>
              <Link href="/universities/wpi/courses" className="block text-xs text-gray-500 hover:underline mt-3">
                All Worcester Polytechnic Institute programs →
              </Link>
            </div>
          </div>
        </div>
