...
export async function generateStaticParams() {
  return (ucsdCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const c = ucsdCourses.find(x => x.slug === slug);
  if (!c) return {};
  return buildMetadata({
    title: `${c.name} at Uc San Diego — Fees, IELTS & Intake for Indian Students 2026`,
    description: `${c.name} at Uc San Diego, ${(c as any).city || c.country} costs ₹${(c.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${c.ieltsMin}+, intakes ${c.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/uc-san-diego/courses/${slug}`,
  });
}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-6 flex-wrap">
        ...
        <Link href="/universities/uc-san-diego/courses" className="hover:text-brand-700">Courses</Link> /
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500 text-sm">UC San Diego · {c.city}, {c.state}</p>
          ...
          <CourseRichContent course={c as any} universityName="Uc San Diego" universitySlug="uc-san-diego" />
          <div className="bg-brand-700 rounded-2xl p-6 text-white text-center">
            <h2 className="text-lg font-bold mb-2">Apply for {c.name} at UC San Diego</h2>
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="uc-san-diego-course" defaultCountry="USA" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">More Programs at UC San Diego</h3>
              <div className="space-y-2">
                {ucsdCourses.filter(x=>x.slug!==slug).slice(0,6).map(x=>(
                  <Link key={x.id} href={`/universities/uc-san-diego/courses/${x.slug}`}
                    className="block text-sm text-brand-700 hover:underline">
                    {x.name} →
                  </Link>
                ))}
              </div>
              <Link href="/universities/uc-san-diego/courses" className="block text-xs text-gray-500 hover:underline mt-3">
                All UC San Diego programs →
              </Link>
            </div>
          </div>
        </div>
