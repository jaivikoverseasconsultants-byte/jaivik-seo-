...
import { lmuCourses } from '@/data/lmu-courses';
import LeadForm from '@/components/LeadForm';
import JsonLd from '@/components/JsonLd';

export const metadata: Metadata = buildMetadata({
  title: 'LMU Munich International Courses – Programs, Fees & IELTS 2026',
  description: `LMU Munich — ${(lmuCourses as unknown as any[]).length} courses for international students. IELTS 6.5+. October & April intakes. Free admission guidance from Jaivik Overseas Consultants.`,
  path: '/universities/lmu-munich/courses',
  keywords: ['LMU courses', 'LMU Munich international', 'study in Germany', 'Germany university'],
});
...
export default function CoursesPage() {
  const courses = lmuCourses as unknown as any[];
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `How many courses does LMU Munich offer for international students?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `LMU Munich offers ${courses.length} programs for international students including Undergraduate, Master's, and PhD degrees.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the minimum IELTS score required at LMU Munich?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The minimum IELTS score at LMU Munich is ${_minIelts}+. High-demand programs may require up to 7.0.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the average tuition fee at LMU Munich?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The average annual tuition at LMU Munich is approximately ${_avgFeeUSD.toLocaleString()} USD (≈ ₹${(_avgFeeUSD * 84 / 100000).toFixed(1)}L INR). Fees vary by program and level.`,
        },
      },
      {
        '@type': 'Question',
        name: `What intake options does LMU Munich offer?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `LMU Munich offers ${_intakesText} intake${_intakeSample.length > 1 ? 's' : ''}. Apply 3–6 months before the intake opening.`,
        },
      },
      {
        '@type': 'Question',
        name: `How can Indian students apply to LMU Munich?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Indian students can apply to LMU Munich in Germany through Jaivik Overseas Consultants — free application guidance, SOP writing, and visa assistance included.`,
        },
      },
    ],
  };
  ...
  const courseListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `LMU Munich — Courses for International Students`,
    description: `${courses.length} programs at LMU Munich, Germany. Min IELTS ${_minIelts}+.`,
    ...
    itemListElement: courses.slice(0, 5).map((c: any, i: number) => ({
      ...
      item: {
        '@type': 'Course',
        name: c.name,
        provider: { '@type': 'CollegeOrUniversity', name: 'LMU Munich' },
  ...
  const schema = {
    ...
    name: 'LMU Munich', sameAs: 'https://www.lmu.de/en/',
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4">
            ...
            <span className="text-white">LMU</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇩🇪 Munich, Germany · #59 QS World Ranking
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">
                LMU Munich — International Courses
              </h1>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { label: 'QS Ranking', value: '#59 QS' },
                  { label: 'Avg PG Fee', value: `€${Math.round(avgFee/1000)}K` },
                  { label: 'Campus', value: 'Munich' },
                ].map(s => (
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="lmu-courses-index" defaultCountry="Germany" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div key={level}>
              ...
              <div className="space-y-3">
                {(lvC as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((c: any) => (
                  <Link key={c.slug} href={`/universities/lmu-munich/courses/${c.slug}`}
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="lmu-courses-sidebar" defaultCountry="Germany" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">Quick Facts — LMU</h3>
              {[
                ['Established', '1472'],
