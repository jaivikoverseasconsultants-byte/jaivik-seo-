...
import { cquCourses, getCquCoursesBySlug } from '@/data/cqu-courses';
...
export async function generateStaticParams() {
  return (cquCourses as unknown as any[]).map((c: any) => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const course = getCquCoursesBySlug(slug);
  if (!course) return {};
  return buildMetadata({
    title: `${course.name} at CQUniversity Australia — Fees in INR, IELTS & Requirements for Indian Students`,
    description: `${course.name} at CQUniversity Australia, ${(course as any).city || course.country} costs ₹${(course.annualINR / 100000).toFixed(1)}L/year for Indian students. IELTS ${course.ieltsMin}+, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/cquniversity/courses/${slug}`,
    keywords: [course.name, 'CQUniversity', 'CQUniversity Australia', 'study in Australia', course.level],
  });
}
    ...
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇦🇺 CQUniversity Australia · {course.campus}
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at CQUniversity Australia — Fees in INR, IELTS &amp; Requirements for Indian Students</h1>
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={`cqu-course-${slug}`} defaultCountry="Australia" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <CourseRichContent course={course as any} universityName="CQUniversity Australia" universitySlug="cquniversity" />
          <div className="bg-brand-700 rounded-2xl p-6 text-white text-center">
            ...
            <p className="text-blue-200 text-sm mb-4">
              Book a free counselling session. Our advisors specialise in CQUniversity admissions for Indian students.
            </p>
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={`cqu-course-${slug}-sidebar`} defaultCountry="Australia" />
            <div className="mt-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <Link href="/universities/cquniversity/courses" className="block text-sm text-brand-700 hover:underline">
                  All CQUniversity Courses →
                </Link>
