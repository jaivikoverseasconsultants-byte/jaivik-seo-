...
export function generateStaticParams() {
  return uiucCourses.map(c => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  const c = uiucCourses.find(x => x.slug === slug);
  if (!c) return {};
  return buildMetadata({
    title: `${c.name} at Uiuc Illinois`,
    description: `${c.name} at Uiuc Illinois, ${(c as any).city || c.country}${feeSentenceINR(c as any, c.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, c as never) ? ` IELTS ${c.ieltsMin}+` : ''}, intakes ${c.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    path: `/universities/uiuc-illinois/courses/${slug}`,
  });
}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-6 flex-wrap">
        ...
        <Link href="/universities/uiuc-illinois/courses" className="hover:text-brand-700">Courses</Link> /
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">{c.name} at Uiuc Illinois — {titleFeeFragment(c as any, c.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
            <p className="text-gray-500 text-sm">University of Illinois Urbana-Champaign · {c.city}, {c.state}</p>
          ...
          <CourseRichContent course={c as any} universityName="Uiuc Illinois" universitySlug="uiuc-illinois" />
          <div className="bg-brand-700 rounded-2xl p-6 text-white text-center">
            <h2 className="text-lg font-bold mb-2">Apply for {c.name} at University of Illinois Urbana-Champaign</h2>
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="uiuc-illinois-course" defaultCountry="USA" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">More Programs at University of Illinois Urbana-Champaign</h3>
              <div className="space-y-2">
                {uiucCourses.filter(x=>x.slug!==slug).slice(0,6).map(x=>(
                  <Link key={x.id} href={`/universities/uiuc-illinois/courses/${x.slug}`}
                    className="block text-sm text-brand-700 hover:underline">
                    {x.name} →
                  </Link>
                ))}
              </div>
              <Link href="/universities/uiuc-illinois/courses" className="block text-xs text-gray-500 hover:underline mt-3">
                All University of Illinois Urbana-Champaign programs →
              </Link>
            </div>
          </div>
        </div>
