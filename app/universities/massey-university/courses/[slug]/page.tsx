...
export async function generateMetadata(
  ...
  return buildMetadata({
    ...
    description: `${course.name} at Massey University, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at Massey University — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                { label: 'Annual Tuition (NZD)', value: feeDisplay(course as any, course.annualNZD, 'NZD') },
            ...
            {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0 && (entryRequirementsVaryByCourse(UNIVERSITY_SLUG) || !!(course as any).englishVerified) ? (
              ...
              <p className="text-sm text-gray-600">
                {publishedEnglishTests(UNIVERSITY_SLUG, course as never).length > 0
                  ? "Massey University publishes one English language requirement across its courses rather than a per-course score."
                  : englishOnRequestNote("Massey University")}{' '}
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                { label: `Tuition Fee × ${course.durationYears} year${course.durationYears !== 1 ? 's' : ''}`, value: feeDisplay(course as any, course.totalNZD, 'NZD'), highlight: true },
                ...
                { label: 'Total Estimated Cost', value: (isFeeVerified(course as any) ? `NZ$${(course.totalNZD + course.livingCostNZD * course.durationYears).toLocaleString()}` : 'On request'), highlight: true },
                { label: 'In Indian Rupees (₹)', value: (isFeeVerified(course as any) ? `₹${((course.totalNZD + course.livingCostNZD * course.durationYears) * RATE_TO_INR.NZD / 100000).toFixed(1)} Lakh` : 'On request'), highlight: true },
              ].map(r => (
