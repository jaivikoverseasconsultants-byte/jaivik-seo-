...
import { tcdCourses, getTcdCourseBySlug } from '@/data/tcd-courses';
...
export async function generateMetadata(
  ...
  const course = getTcdCourseBySlug(slug);
  if (!course) return {};
  const fee = (course as any).annualEUR || (course as any).annualUSD || 0;
  return buildMetadata({
    title: `${course.name} at Trinity College Dublin`,
    description: `${course.name} at Trinity College Dublin, ${(course as any).city || course.country}${feeSentenceINR(course as any, course.annualINR)}${hasPublishedIelts(UNIVERSITY_SLUG, course as never) ? ` IELTS ${course.ieltsMin}+` : ''}, intakes ${course.intakeMonths.join(' & ')}. Apply with Jaivik Overseas — 13 years expertise, 99% visa success.`,
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-blue-200 text-xs mb-4 flex-wrap">
            ...
            <Link href="/universities/trinity-college-dublin/courses" className="hover:text-white">Courses</Link> /
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-3">{course.name} at Trinity College Dublin — {titleFeeFragment(course as any, course.annualINR)}IELTS &amp; Requirements for Indian Students</h1>
          <p className="text-blue-100 text-lg">
            Trinity College Dublin · Dublin, Ireland · {course.duration}
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                  { label: 'Annual Fee', value: feeDisplay(course as any, fee, 'EUR'), sub: feeDisplayINRLakh(course as any, feeINRLakh, '/year') },
                  { label: 'Total Fees', value: feeDisplay(course as any, totalFee, 'EUR'), sub: `${course.durationYears} year(s)` },
                  ...
                  { label: 'Campus', value: course.campus?.split(',')[0] || 'Dublin', sub: 'Ireland' },
                ].map(({ label, value, sub }) => (
            ...
            {/* Why Trinity Dublin */}
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-4">Why Choose Trinity College Dublin?</h2>
          ...
          <CourseRichContent course={course as any} universityName="Trinity College Dublin" universitySlug="trinity-college-dublin" />
          <div className="bg-brand-700 text-white rounded-2xl p-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">€{(course as any)['livingCostEUR']?.toLocaleString() || 'N/A'}/year</span>
                </div>
                <div className="flex justify-between">
            ...
            <Link href="/universities/trinity-college-dublin/courses" className="flex items-center gap-2 text-brand-700 hover:text-brand-900 font-medium text-sm">
              ← All Trinity Dublin Courses
            </Link>
          </div>
        </div>
      </section>
