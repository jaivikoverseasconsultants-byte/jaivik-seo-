...
export function generateStaticParams() {
  return ntuCourses.map(c => ({ slug: c.slug }));
}
...
export async function generateMetadata(
  ...
  return buildMetadata({
    title: `${c.name} at Nanyang Technological University 2026 – Fees, IELTS & Requirements`,
    description: `${c.name} at Nanyang Technological University: ${c.duration}, S$${(c.annualSGD/1000).toFixed(0)}K SGD/yr. IELTS ${c.ieltsMin}+. Apply via Jaivik Overseas Consultants.`,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-6 flex-wrap">
        ...
        <Link href="/universities/nanyang-technological-university/courses" className="hover:text-brand-700">Courses</Link> /
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="nanyang-technological-university-course" defaultCountry="Singapore" />
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3 text-sm">More Programs at Nanyang Technological University</h3>
              <div className="space-y-2">
                {ntuCourses.filter(x=>x.slug!==slug).slice(0,6).map(x=>(
                  <Link key={x.id} href={`/universities/nanyang-technological-university/courses/${x.slug}`}
                    className="block text-sm text-brand-700 hover:underline">
                    {x.name} →
                  </Link>
                ))}
              </div>
              <Link href="/universities/nanyang-technological-university/courses" className="block text-xs text-gray-500 hover:underline mt-3">
                All Nanyang Technological University programs →
              </Link>
            </div>
          </div>
        </div>
