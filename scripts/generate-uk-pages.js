/**
 * UK university page generator.
 * Usage: node scripts/generate-uk-pages.js <uni-id>
 * Reads: data/scraped/uk/<uni-id>.json
...
const GBP_TO_INR = 107; // 1 GBP ≈ 107 INR
const LIVING_GBP = 12000;
const LIVING_USD = Math.round(LIVING_GBP * GBP_TO_USD);
const LIVING_INR = Math.round(LIVING_GBP * GBP_TO_INR);
...
function generatePages(uniId) {
  ...
  const inFile = path.join(__dirname, `../data/scraped/uk/${uniId}.json`);
  if (!fs.existsSync(inFile)) { console.error(`No data found: ${inFile}\nRun scraper first.`); process.exit(1); }
  ...
  const mapped = courses.map((c, i) => {
    const annualGBP = c.annualGBP || config.fees.default;
    const annualUSD = c.annualUSD || Math.round(annualGBP * GBP_TO_USD);
    const annualINR = Math.round(annualUSD * 84);
    const livingINR = LIVING_INR;
    ...
    return {
      ...
      annualUSD,
      annualINR,
      totalGBP: c.totalGBP || Math.round(annualGBP * durationYears),
...
// Source: data/scraped/uk/${uniId}.json
...
`;
  ...
  const avgFee = Math.round(mapped.reduce((s, c) => s + c.annualGBP, 0) / (mapped.length || 1));
...
export async function generateMetadata(
  ...
  return buildMetadata({
    ...
    description: \`\${course.name} at ${config.name}. Annual fee £\${course.annualGBP.toLocaleString()} (\${course.durationYears} year\${course.durationYears !== 1 ? 's' : ''}). IELTS \${course.ieltsMin}+. Intake: \${course.intakeMonths.join(' & ')}. Free guidance from Jaivik Overseas Consultants.\`,
    path: \`/universities/${config.slug}/courses/\${slug}\`,
    keywords: [course.name, '${config.shortName}', '${config.name}', 'study in UK', course.level],
  });
}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source={\`${uniId}-course-\${slug}\`} defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'In Indian Rupees (₹)', value: \`₹\${((course.totalGBP + course.livingCostGBP * course.durationYears) * ${GBP_TO_INR} / 100000).toFixed(1)} Lakh\`, highlight: true },
              ].map(r => (
        ...
        <div className="space-y-5">
          <div className="sticky top-20">
            <LeadForm source={\`${uniId}-course-\${slug}-sidebar\`} defaultCountry="UK" />
...
export const metadata: Metadata = buildMetadata({
  ...
  keywords: ['${config.shortName} courses', '${config.name} international', '${config.shortName} fees', 'study in UK', 'UK university'],
});
...
export default function CoursesPage() {
  ...
  const schema = {
    ...
    name: '${config.name}',
    sameAs: '${config.sameAs}',
    address: { '@type': 'PostalAddress', addressLocality: '${config.city}', addressRegion: '${config.state}', addressCountry: 'GB' },
  };
      ...
      <section className="bg-gradient-to-br from-brand-700 to-brand-900 text-white py-14 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              <div className="inline-flex items-center gap-2 bg-gold-500/20 text-gold-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
                🇬🇧 ${config.city}, United Kingdom${config.qsRanking && config.qsRanking < 1400 ? ` · #${config.qsRanking} QS World Ranking` : ''}
              </div>
              <h1 className="text-3xl md:text-4xl font-bold mb-3">
                ${config.name} — International Courses
              </h1>
              <p className="text-blue-200 text-lg mb-5">
                {totalCourses} programs · Avg £\${avgFee.toLocaleString()}/yr · IELTS ${config.ieltsDefault}+ · ${intakesStr} intakes
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { label: 'Campus', value: '${config.campus.split(' ')[0]}' },
                ].map(s => (
                  <div key={s.label} className="bg-white/10 rounded-xl p-3 text-center">
            ...
            <div className="bg-white rounded-2xl p-5">
              <LeadForm source="${uniId}-courses-index" defaultCountry="UK" compact />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div>
          <div className="sticky top-20 space-y-5">
            <LeadForm source="${uniId}-courses-sidebar" defaultCountry="UK" />
...
if (!uniId) { console.error('Usage: node generate-uk-pages.js <uni-id>'); process.exit(1); }
generatePages(uniId);
