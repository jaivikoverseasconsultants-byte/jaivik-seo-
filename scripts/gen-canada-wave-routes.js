...
const PUBLISHED_TESTS = ${JSON.stringify(publishedTests(u))};
                ...
                {lvCourses.slice().sort((a, b) => a.name.localeCompare(b.name)).map((c) => (
                  <Link key={c.slug} href={\`/universities/${u.slug}/courses/\${c.slug}\`}
                    ...
                    <div className="ml-4 text-right flex-shrink-0">
                      ...
                      {englishTests(c).some((t) => t.label === 'IELTS Academic') && <p className="text-xs text-gray-500">IELTS {c.ieltsMin}+</p>}
                    </div>
                  </Link>
                ))}
              </div>
